...
import { buildRoleChangedEmail } from '../services/email/templates/role-changed.js'
...
import { buildBotAlertEmail } from '../services/email/templates/bot-alert.js'
...
describe('buildBotAlertEmail', () => {
  ...
  test('disconnected: html contiene icon SVG',  () => assert.ok(disconnected.html.includes('<svg')))
  test('reconnected: html contiene icon SVG',   () => assert.ok(reconnected.html.includes('<svg')))
})

import { buildSubbotAlertEmail } from '../services/email/templates/subbot-alert.js'
...
import { buildAporteReceivedEmail } from '../services/email/templates/aporte-received.js'
...
import { buildLoginNewDeviceEmail } from '../services/email/templates/login-new-device.js'
...
import { buildAccountDeletedEmail } from '../services/email/templates/account-deleted.js'
...
import { buildAporteAceptadoEmail } from '../services/email/templates/aporte-aceptado.js'
import { buildAporteRechazadoEmail } from '../services/email/templates/aporte-rechazado.js'
import { buildAportePendienteEmail } from '../services/email/templates/aporte-pendiente.js'
...
import { buildMaintenanceNoticeEmail } from '../services/email/templates/maintenance-notice.js'
import { buildAccountSuspendedEmail } from '../services/email/templates/account-suspended.js'
import { buildAccountReactivatedEmail } from '../services/email/templates/account-reactivated.js'
import { buildSubbotCreatedEmail } from '../services/email/templates/subbot-created.js'
import { buildSubbotDeletedEmail } from '../services/email/templates/subbot-deleted.js'
import { buildTwoFactorCodeEmail } from '../services/email/templates/two-factor-code.js'
import { buildSystemAlertEmail } from '../services/email/templates/system-alert.js'

describe('maintenance-notice', async () => {
  const r = await buildMaintenanceNoticeEmail({ startTime: '03:00 AM', durationMinutes: 30, reportHtml: '' })
  ...
  test('html es type warning (color dorado)', () => assert.ok(r.html.includes('#d97706')))
  test('html contiene ícono SVG', () => assert.ok(r.html.includes('<svg')))
  test('affectedServices se listan', async () => {
    const r2 = await buildMaintenanceNoticeEmail({ startTime: 'T', durationMinutes: 10, affectedServices: ['Panel', 'API'], reportHtml: '' })
...
describe('account-suspended', () => {
  ...
  test('html es type danger (color rojo)', () => assert.ok(r.html.includes('#dc2626')))
...
describe('account-reactivated', () => {
  ...
  test('html contiene username', () => assert.ok(r.html.includes('juan')))
  test('html es type success (color verde)', () => assert.ok(r.html.includes('#16a34a')))
})
...
describe('subbot-created', () => {
  ...
  test('html es type success', () => assert.ok(r.html.includes('#16a34a')))
})

describe('subbot-deleted', () => {
  const r = buildSubbotDeletedEmail({ subbotName: 'TestBot', deletedBy: 'Admin' })
  test('html contiene nombre', () => assert.ok(r.html.includes('TestBot')))
  test('html es type danger', () => assert.ok(r.html.includes('#dc2626')))
})
...
describe('two-factor-code', () => {
  ...
  test('html es type info (azul)', () => assert.ok(r.html.includes('#2563eb')))
