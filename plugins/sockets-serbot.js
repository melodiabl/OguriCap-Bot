    ...
    try {
      ...
      setTimeout(async () => {
        ...
        if (!sock.user && !sock.isInit && getConnReadyState(sock) !== ws.OPEN && !hasCreds) {
          ...
          try {
            const { emitSubbotDeleted, emitSubbotDisconnected } = await import('../services/socket-io.js')
        ...
        if (connection == `open`) {
          ...
          try {
            ...
            try {
              ...
              try {
                const { emitSubbotConnected, emitSubbotStatus, emitSubbotUpdated, emitSubbotCreated } = await import('../services/socket-io.js')
