...
export function initSocketIO(httpServer) {
  ...
  const corsOrigin = process.env.SOCKET_IO_CORS_ORIGIN || process.env.PANEL_CORS_ORIGIN || process.env.CORS_ORIGIN || '*'
  const corsOrigins =
    corsOrigin === '*' ? true : corsOrigin.split(',').map((v) => v.trim()).filter(Boolean)
  ...
  io.on('connection', async (socket) => {
    ...
    // Autenticación del socket para prevenir filtraciones (ej. notificaciones push sin login)
    const token = socket.handshake.auth?.token;
    if (token) {
      try {
        const panelApiKey = String(process.env.PANEL_API_KEY || '').trim();
        if (panelApiKey && token === panelApiKey) {
        ...
        // Token inválido, cliente no se une a la sala 'authenticated'
      }
    }
...
/**
 * Emite cuando un subbot es actualizado
 */
export function emitSubbotUpdated(subbot) {
  if (!io) return
  io.emit(SOCKET_EVENTS.SUBBOT_UPDATED, { 
    subbot, 
    timestamp: new Date().toISOString() 
  })
}
...
function buildSubbotStatusList() {
  const conns = Array.isArray(global.conns) ? global.conns : []
  const db = global.db?.data || {}
  const records = db.subbots && typeof db.subbots === 'object' ? db.subbots : {}

  // Build a map: any identifier a conn can have → { online, phone }
  const onlineMap = new Map()
  for (const conn of conns) {
    const online = isConnOnline(conn)
    const phone = conn?.user?.jid || conn?.user?.id || conn?.authState?.creds?.me?.jid || conn?.authState?.creds?.me?.id || null
    const phoneDigits = phone ? String(phone).split('@')[0].replace(/[^0-9]/g, '') : null
    const subbotCode = conn?.subbotCode ? String(conn.subbotCode).trim() : null
    const sessionBase = conn?.sessionPath ? String(conn.sessionPath).split('/').pop() : null
    for (const key of [subbotCode, sessionBase, phoneDigits].filter(Boolean)) {
      if (!onlineMap.has(key)) onlineMap.set(key, { online, phone })
    }
  }

  const getConnInfo = (record) => {
    const code = String(record.codigo || record.code || '').trim()
    const codeDigits = code.replace(/[^0-9]/g, '')
    const aliasDir = String(record.alias_dir || record.aliasDir || '').trim()
    const sessionDir = String(record.session_dir || '').trim()
    const numero = String(record.numero || '').replace(/[^0-9]/g, '')
    for (const key of [code, codeDigits, aliasDir, sessionDir, numero].filter(Boolean)) {
      if (onlineMap.has(key)) return onlineMap.get(key)
    }
    return { online: false, phone: null }
  }
  ...
  for (const [key, record] of Object.entries(records)) {
    const code = String(record.codigo || record.code || key).trim()
    seen.add(code)
    const connInfo = getConnInfo(record)
    const isOnline = connInfo.online
    const hasPendingAuth = !isOnline && (record.qr_data || record.pairingCode)
    result.push({
      subbotCode: code,
      isOnline,
      status: isOnline ? 'activo' : 'inactivo',
      connectionState: isOnline ? 'connected' : (hasPendingAuth ? 'needs_auth' : 'disconnected'),
      phone: connInfo.phone || record.numero || null,
      ...
      tipo: record.tipo || record.type || 'qr',
      ...
      fecha_creacion: record.fecha_creacion || record.created_at || null,
      id: record.id,
    })
  }

  // Include any active connections not in panelDb
  for (const conn of conns) {
    const phone = conn?.user?.jid || conn?.user?.id || null
    const code = conn?.subbotCode || (phone ? phone.split('@')[0] : null)
    if (!code || seen.has(String(code).trim())) continue
    result.push({
      subbotCode: String(code).trim(),
      isOnline: isConnOnline(conn),
      status: isConnOnline(conn) ? 'activo' : 'inactivo',
      connectionState: isConnOnline(conn) ? 'connected' : 'disconnected',
      phone: phone || null,
    })
  }
...
export function emitSubbotStatus(subbots) {
  if (!io) return
  const list = Array.isArray(subbots) ? subbots : buildSubbotStatusList()
...
function emitSubbotStatusTo(socket) {
  socket.emit(SOCKET_EVENTS.SUBBOT_STATUS, { subbots: buildSubbotStatusList(), timestamp: new Date().toISOString() })
}
...
function getDashboardStatsSnapshot() {
  ...
  const db = global.db?.data || {}
  // Lee desde estructura migrada; usa panel.* como fallback para campos no migrados (logs, dailyMetrics)
  const panel = db.panel || {}

  const logs = Array.isArray(db.logs) ? db.logs : (Array.isArray(panel.logs) ? panel.logs : [])
  ...
  const dailyMetrics = (db.dailyMetrics && typeof db.dailyMetrics === 'object')
    ? db.dailyMetrics
    : (panel?.dailyMetrics && typeof panel.dailyMetrics === 'object' ? panel.dailyMetrics : {})
  ...
  // Usuarios del panel (estructura migrada)
  const usuariosMap = db.usuarios && typeof db.usuarios === 'object' ? db.usuarios : (panel.users || {})
  const panelUsers = Object.values(usuariosMap)
  ...
  const groupsMap = db.groups && typeof db.groups === 'object' ? db.groups : (panel.groups || {})
  const totalGrupos = Object.keys(groupsMap).length || groupJids.length
  ...
  const pedidosRaw = db.pedidos && typeof db.pedidos === 'object' ? db.pedidos : (panel.pedidos || {})
  ...
  const subbots = db.subbots && typeof db.subbots === 'object' ? db.subbots : (panel.subbots || {})
  const totalSubbots = Object.keys(subbots).length || (Array.isArray(global.conns) ? global.conns.length : 0)
...
export function emitNotification(notification) {
  ...
  const titulo = String(raw.titulo ?? raw.title ?? 'Notificación')
  const mensaje = String(raw.mensaje ?? raw.message ?? '')
  const tipo = String(raw.tipo ?? raw.type ?? 'info')
  const categoria = String(raw.categoria ?? raw.category ?? 'general')
  
  // Roles objetivo: si no se especifica, mostrar a todos
  const targetRoles = Array.isArray(raw.targetRoles) 
    ? raw.targetRoles 
    : raw.targetRoles 
      ? [raw.targetRoles] 
      : null
  const para = raw.para ?? raw.for ?? null

  const leida = Boolean(raw.leida ?? raw.read ?? false)
  const fecha_creacion = String(raw.fecha_creacion ?? raw.created_at ?? raw.timestamp ?? nowIso)
  const created_at = String(raw.created_at ?? fecha_creacion)
  const updated_at = String(raw.updated_at ?? created_at)
  const timestamp = String(raw.timestamp ?? nowIso)

  io.to('authenticated').emit(SOCKET_EVENTS.NOTIFICATION, {
    ...raw,
    id,
    titulo,
    mensaje,
...
export function emitLogEntry(log) {
  ...
  io.to('authenticated').emit(SOCKET_EVENTS.LOG_ENTRY, { ...log, timestamp: ts })
}

/**
 * Emite una línea cruda del terminal (stdout/stderr) con ANSI intacto
 */
export function emitTerminalLine(payload) {
  if (!io) return
  const ts = payload?.timestamp || new Date().toISOString()
  io.to('authenticated').emit(SOCKET_EVENTS.TERMINAL_LINE, { ...payload, timestamp: ts })
}
...
/**
 * Conecta el Event Bus con Socket.IO para emitir eventos en tiempo real
 */
export async function connectEventBusToSocket() {
  try {
    const eventBus = (await import('./event-bus.js')).default
    const { EVENTS } = await import('./event-bus.js')
    
    console.log('[Socket.IO] Conectando Event Bus...')
    
    // Escuchar eventos de sincronización de grupos
    eventBus.on(EVENTS.GROUP_SYNC_START, () => {
      emit(SOCKET_EVENTS.GRUPOS_SYNCED, { status: 'started', timestamp: Date.now() })
    })
    
    eventBus.on(EVENTS.GROUP_SYNC_COMPLETE, (data) => {
      emit(SOCKET_EVENTS.GRUPOS_SYNCED, { 
        status: 'completed', 
        synced: data.synced, 
    ...
    eventBus.on(EVENTS.GROUP_SYNC_ERROR, (data) => {
      emit(SOCKET_EVENTS.GRUPOS_SYNCED, { 
        status: 'error', 
        error: data.error,
        timestamp: Date.now() 
    ...
    eventBus.on(EVENTS.GROUP_UPDATED, (group) => {
      emitGrupoUpdated(group)
    })
    
    // Escuchar eventos de pedidos
    eventBus.on(EVENTS.PEDIDO_CREATED, (pedido) => {
      emitPedidoCreated(pedido)
    })
    
    eventBus.on(EVENTS.PEDIDO_UPDATED, (pedido) => {
      emitPedidoUpdated(pedido)
    })
    
    eventBus.on(EVENTS.PEDIDO_DELETED, (data) => {
      emitPedidoDeleted(data.id)
    })
    
    // Escuchar eventos de aportes
    eventBus.on(EVENTS.APORTE_CREATED, (aporte) => {
      emitAporteCreated(aporte)
    })
    
    eventBus.on(EVENTS.APORTE_UPDATED, (aporte) => {
      emitAporteUpdated(aporte)
    })
    
    eventBus.on(EVENTS.APORTE_DELETED, (data) => {
      emitAporteDeleted(data.id)
    })
    ...
    console.log('[Socket.IO] Event Bus conectado exitosamente')
  } catch (err) {
    console.error('[Socket.IO] Error conectando Event Bus:', err)
  }
}
