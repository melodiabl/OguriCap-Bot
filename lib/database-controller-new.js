 ...
 * 
 * Simplified controller that uses only PostgreSQL
...
class DatabaseController extends EventEmitter {
  ...
  async initialize() {
    try {
      console.log('🔄 Initializing PostgreSQL Database Controller...');
      ...
      console.log('✅ Database Controller initialized successfully');
      
      this.emit('initialized');
      return true;
      
    } catch (error) {
      console.error('❌ Failed to initialize Database Controller:', error.message);
      throw error;
    }
  }
  ...
  async loadData() {
    try {
      ...
      // Load users
      const users = await this.adapter.query('SELECT * FROM whatsapp_users');
      this.data.users = {};
      ...
      // Load chats
      const chats = await this.adapter.query('SELECT * FROM chats');
      this.data.chats = {};
      ...
      // Load settings
      const settings = await this.adapter.query('SELECT * FROM settings');
      this.data.settings = {};
      ...
      // Load panel users
      const usuarios = await this.adapter.query('SELECT * FROM usuarios');
      this.data.usuarios = {};
