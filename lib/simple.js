...
export function makeWASocket(connectionOptions, options = {}) {
    ...
    const sock = Object.defineProperties(conn, {
        ...
        isConnected: {
            // Compatible con baileys 6.x (ws.socket.readyState) y 7.x (ws.isOpen getter)
            get() {
                const ws = this.ws
                if (!ws) return false
                if (typeof ws.isOpen === 'boolean') return ws.isOpen
                if (ws.socket?.readyState != null) return ws.socket.readyState === 1
                if (ws.readyState != null) return ws.readyState === 1
                return false
            },
        },
        logger: {
...
END:VCARD
    ...
    dynamicButtons.push({
        ...
        sendList: {
            async value(
                ...
                const cleanSections = sections
                    .map((section) => {
                        const rowsIn = Array.isArray(section?.rows) ? section.rows : [];
                        const rows = rowsIn
                            .map((row) => ({
                        ...
                        buttons: [
                            {
                                name: "single_select",
                                buttonParamsJson: JSON.stringify({
                                    title: String(buttonText || "Ver opciones"),
        ...
        downloadMToFile: {
            /**
             * Stream-download media message to a file.
             ...
             * @return {Promise<{ filename: string, bytes: number }>}
             */
            async value(m, type, dest, options = {}) {
                if (!dest || typeof dest !== "string") throw new Error("dest path requerido");
                if (!m || !(m.url || m.directPath)) throw new Error("Media no descargable");
                ...
                const armTimeout = () => {
                    ...
                    timeout = setTimeout(() => {
                        try {
                            out.destroy(new Error("Download timeout"));
                        } catch { }
                    }, timeoutMs);
                };
                ...
                try {
                    for await (const chunk of stream) {
                        ...
                        if (maxBytes > 0 && bytes > maxBytes) {
                            throw new Error("Archivo excede el limite");
                        }
                        if (!out.write(chunk)) {
    ...
    // Wrapper de groupMetadata: normaliza participantes para cualquier fork/versión de baileys
    // 6.x: { jid, lid, admin } — 7.x: { id (@lid), phoneNumber (@s.whatsapp.net), admin }
    ...
    sock.groupMetadata = async (jid) => {
        ...
        meta.participants = meta.participants.map(p => {
            if (p.jid) return p  // ya normalizado (6.x)
            return {
                ...p,
                jid: p.phoneNumber || p.id,
                lid: p.id?.endsWith('@lid') ? p.id : p.lid,
            }
        })
        return meta
    }
...
export function serialize() {
    const MediaType = ["imageMessage", "videoMessage", "ptvMessage", "audioMessage", "stickerMessage", "documentMessage"];
        ...
        isBaileys: {
            get() {
                try {
                    ...
                    // "3EB0" es un prefijo común de WhatsApp (usuarios normales),
                    // no debe considerarse "baileys/bot" o el handler ignora mensajes reales.
                    // En varios forks, la longitud del ID puede variar. Confiar en el prefijo.
                    const isKnownBaileys =
                        messageId.startsWith("BAE5") ||
                        messageId.startsWith("B24E");

                    // Algunos bots/forks usan IDs personalizados (prefijos/formatos)
                    // para evitar loops; tratarlos como bot (antibot).
                    ...
                    return isKnownBaileys || hasCustomPrefix || isMysticPattern || isSukiPattern;
                } catch (e) {
                    ...
                    return Object.defineProperties(
                        ...
                        download: {
                            value(saveToFile = false) {
                                ...
                                const downloadType =
                                    mtype === "ptvMessage" ? "video" : mtype?.replace(/message/i, "");
            ...
            get() {
                try {
                    ...
                    const readText = (obj) => {
                        ...
                        const selectedButtonId = obj?.selectedButtonId;
                        const selectedRowId = obj?.singleSelectReply?.selectedRowId;
                        const selectedTemplateId = obj?.selectedId;

                        let nativeFlowSelected = "";
                        const paramsJson =
                            obj?.nativeFlowResponseMessage?.paramsJson ||
                            obj?.interactiveResponseMessage?.nativeFlowResponseMessage?.paramsJson ||
                            obj?.interactiveResponseMessage?.paramsJson ||
                            obj?.paramsJson ||
                            "";
                        if (typeof paramsJson === "string" && paramsJson) {
                            try {
                                ...
                                nativeFlowSelected =
                                    ...
                                    (typeof parsed?.selectedRowId === "string" && parsed.selectedRowId) ||
                                    (typeof parsed?.rowId === "string" && parsed.rowId) ||
                                    (typeof parsed?.selectedButtonId === "string" && parsed.selectedButtonId) ||
                                    "";
                            } catch { }
                        }

                        // IMPORTANTE: para respuestas interactivas, prioriza los IDs (comandos)
                        // sobre el texto visible del botón/lista.
                        const interactiveId =
                            selectedButtonId ||
                            selectedRowId ||
                            selectedTemplateId ||
                        ...
                        const body =
                            ...
                            obj?.contentText ||
                            obj?.body?.text ||
                            obj?.hydratedTemplate?.hydratedContentText ||
                    ...
                    const msg = this.msg;
                    let body = readText(msg);

                    // Si vino envuelto (ephemeral/viewOnce/etc), extraer contenido real para capturar captions
                    if (!body) {
                        try {
                            const extracted =
                            ...
                            if (extracted && typeof extracted === "object") {
                                const innerType = Object.keys(extracted)[0];
                                if (innerType) body = readText(extracted?.[innerType]);
                            }
                        } catch { }

                        // Fallback: algunos wrappers traen el mensaje como `msg.message`
                        if (!body && msg?.message && typeof msg.message === "object") {
                            const innerType = Object.keys(msg.message)[0];
                            if (innerType) body = readText(msg.message?.[innerType]);
                        }
                    }
