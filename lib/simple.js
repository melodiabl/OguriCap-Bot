    ...
    dynamicButtons.push({
        ...
        sendButton: {
            async value(
                ...
                const hasMediaAttachment = Boolean(img || video);
                const headerObj = {
                    hasMediaAttachment,
                ...
                const msgL = generateWAMessageFromContent(jid, content, {
                    userJid: conn.user.jid,
                    quoted,
                });
            ...
            async value(
                ...
                const sections = (Array.isArray(listSections) ? listSections : [])
                    .map((sec) => {
                        ...
                        const sectionTitle = typeof sec?.title === "string" ? sec.title.trim() : "";
                        return rows.length ? { title: sectionTitle, rows } : null;
                    })
                    .filter(Boolean);
...
export function serialize() {
    ...
    return Object.defineProperties(proto.WebMessageInfo.prototype, {
        ...
        isBaileys: {
            get() {
                try {
                    ...
                    // BAE5/B24E: típicamente IDs de mensajes generados por Baileys (salientes).
                    // En algunos clientes/escenarios pueden aparecer en entrantes: no bloquear plugins si no es fromMe.
                    const isKnownBaileys =
                        !!this.key?.fromMe &&
                        (messageId.startsWith("BAE5") || messageId.startsWith("B24E"));
        ...
        text: {
            get() {
                try {
                    const rawMsg = this.msg;
                    if (typeof this._text === "string") return this._text;
                    if (!rawMsg) return "";
                    if (typeof rawMsg === "string") return rawMsg;

                    // Unwrap common wrappers (viewOnce/ephemeral) to reach the real content
                    let msg = rawMsg;
                    try {
                        if (rawMsg?.message && typeof rawMsg.message === "object") {
                            const extracted = extractMessageContent(rawMsg.message) || rawMsg.message;
                            if (extracted && typeof extracted === "object") {
                                ...
                                const k = keys[0];
                                if (k && extracted[k]) msg = extracted[k];
                            }
                        }
                    } catch { }
                    ...
                    push(msg?.text);
                    push(msg?.caption);
                    push(msg?.contentText);
                    push(msg?.selectedDisplayText);

                    // Classic list response
                    push(msg?.singleSelectReply?.selectedRowId);
                    push(msg?.singleSelectReply?.selectedDisplayText);
                    push(msg?.listResponseMessage?.singleSelectReply?.selectedRowId);
                    push(msg?.listResponseMessage?.singleSelectReply?.selectedDisplayText);

                    // Buttons response (legacy)
                    push(msg?.buttonsResponseMessage?.selectedButtonId);
                    push(msg?.buttonsResponseMessage?.selectedDisplayText);
                    push(msg?.selectedButtonId);
                    push(msg?.selectedButtonDisplayText);

                    // Template button response
                    push(msg?.templateButtonReplyMessage?.selectedId);
                    push(msg?.templateButtonReplyMessage?.selectedDisplayText);
                    push(msg?.selectedId);

                    // Native flow / interactive response (new lists/buttons)
                    const paramsJson =
                        msg?.nativeFlowResponseMessage?.paramsJson ||
                        msg?.nativeFlowResponseMessage?.buttonParamsJson ||
                        msg?.interactiveResponseMessage?.nativeFlowResponseMessage?.paramsJson ||
                        msg?.interactiveResponseMessage?.nativeFlowResponseMessage?.buttonParamsJson ||
                    ...
                    if (typeof paramsJson === "string" && paramsJson.trim()) {
                        try {
                            ...
                            const tryExtract = (obj) => {
                                ...
                                push(obj?.id);
                                push(obj?.rowId);
                                push(obj?.selectedRowId);
                                push(obj?.selectedButtonId);
                                push(obj?.buttonId);
                                push(obj?.value);
                                push(obj?.display_text);
                                push(obj?.title);
                            };
                            if (Array.isArray(parsed)) {
                                for (const it of parsed) tryExtract(it);
                            } else {
                                tryExtract(parsed);
                            }
                        } catch { }
                    ...
                    // Fallback: hydrated template content text (older forks)
                    push(msg?.hydratedTemplate?.hydratedContentText);
