  ...
  panelServer = http.createServer(async (req, res) => {
    try {
      ...
      if (pathname === '/api/community/users' && method === 'GET') {
        ...
        const communityUsers = Object.entries(whatsappUsers).map(([jid, userData]) => {
      ...
      if (pathname === '/api/community/stats' && method === 'GET') {
        ...
        const totalUsers = Object.keys(whatsappUsers).length
        const activeUsers = Object.values(whatsappUsers).filter(user => {
          const lastSeen = user?.lastSeen || user?.updated_at
          return lastSeen && (Date.now() - new Date(lastSeen).getTime()) < 7 * 24 * 60 * 60 * 1000
        }).length
        
        const bannedUsers = Object.values(chats).filter(chat => chat?.isBanned).length
        
        const today = new Date().toISOString().slice(0, 10)
        const newUsersToday = Object.values(whatsappUsers).filter(user => {
        ...
        const topUsers = Object.entries(whatsappUsers)
          .map(([jid, userData]) => ({
            jid,
            name: userData?.name || userData?.pushName,
            pushName: userData?.pushName,
      ...
      if (pathname === '/api/grupos/debug' && method === 'GET') {
        ...
        try {
          ...
          // Error getting groups from WhatsApp
        }
