  ...
  panelServer = http.createServer(async (req, res) => {
    try {
      ...
      if (pathname === '/api/resources/stats' && method === 'GET') {
        ...
        const current = {
          ...
          bot: {
            connection: {
              ...
              phoneNumber: global.conn?.user?.jid ? String(global.conn.user.jid).split('@')[0] : null,
              qrStatus: global.panelApiMainQr ? 'available' : null
            },
            database: {
              users: Object.keys(panelDb?.users || {}).length,
              groups: Object.keys(panelDb?.groups || {}).length,
              chats: Object.keys(global.db?.data?.chats || {}).length
            },
            subbots: {
              total: Object.keys(panelDb?.subbots || {}).length,
              connected: Object.values(panelDb?.subbots || {}).filter(s => {
                const code = s.codigo || s.code
                return code && findConnBySubbotCode(code)?.user
              }).length
            }
          },
          system: {
        ...
        const alerts = {
          cpu: current.cpu.usage > 90 ? 'critical' : current.cpu.usage > 70 ? 'warning' : 'normal',
          memory: current.memory.usage > 90 ? 'critical' : current.memory.usage > 70 ? 'warning' : 'normal',
          disk: current.disk.usage > 90 ? 'critical' : current.disk.usage > 70 ? 'warning' : 'normal',
          temperature: 'normal'
        }
      ...
      if (pathname === '/api/resources/history' && method === 'GET') {
        ...
        // Generar historial con variaciones pequeñas de los valores actuales
        const history = Array.from({ length: limit }, (_, i) => {
          const variation = (Math.random() - 0.5) * 10 // Variación de ±5%
          return {
            timestamp: new Date(Date.now() - i * 60000).toISOString(),
            cpu: Math.max(0, Math.min(100, cpuUsage + variation)),
            memory: Math.max(0, Math.min(100, (usedMem / totalMem) * 100 + variation)),
            disk: Math.max(0, Math.min(100, diskInfo.usage + variation))
          }
        }).reverse()
      ...
      if (pathname === '/api/system/metrics' && method === 'GET') {
        ...
        const metrics = {
          ...
          disk: diskInfo.usage,
          network: { 
            rx: 0, // Requiere librerías adicionales para datos reales
            tx: 0 
          },
          uptime: process.uptime(),
          timestamp: new Date().toISOString()
      ...
      if (pathname === '/api/system/metrics/history' && method === 'GET') {
        ...
        const points = Math.min(100, Math.floor(timeRange / 60000)) // Max 100 points, 1 per minute
        ...
        const history = Array.from({ length: points }, (_, i) => {
          ...
          return {
            ...
            memory: Math.max(0, Math.min(100, (usedMem / totalMem) * 100 + variation)),
            disk: Math.max(0, Math.min(100, diskInfo.usage + variation)),
            network: { rx: Math.random() * 100, tx: Math.random() * 100 }
          }
        }).reverse()
      ...
      if (pathname === '/api/logs/search' && method === 'GET') {
        ...
        logs.sort((a, b) => new Date(b.timestamp || b.fecha || 0).getTime() - new Date(a.timestamp || a.fecha || 0).getTime())
        
        const result = paginateArray(logs, { page, limit })
        
        // Asegurar que cada log tenga la estructura correcta
        result.items = result.items.map(log => ({
          id: log.id || Math.random().toString(36),
          timestamp: log.timestamp || log.fecha || new Date().toISOString(),
          level: log.level || 'info',
          category: log.category || log.fuente || 'system',
          message: log.message || log.mensaje || '',
          data: log.data || log.metadata || log.meta || {},
          pid: log.pid || process.pid,
          hostname: log.hostname || os.hostname(),
          stack: log.stack || [],
          source: log.source || log.fuente || 'system',
          user: log.user || log.usuario || null,
          ip: log.ip || null
        }))
        
        return json(res, 200, { logs: result.items, ...result.pagination })
      }
      ...
      if (pathname === '/api/logs/stats' && method === 'GET') {
        ...
        const stats = {
          ...
          warnCount: logs.filter(l => l.level === 'warn' || l.level === 'warning').length,
          infoCount: logs.filter(l => l.level === 'info').length,
          debugCount: logs.filter(l => l.level === 'debug').length,
          traceCount: logs.filter(l => l.level === 'trace').length,
          filesCreated: 1,
          filesRotated: 0,
          filesCompressed: 0,
          lastLogTime: logs.length > 0 ? logs[logs.length - 1]?.timestamp || logs[logs.length - 1]?.fecha : null,
          uptime: process.uptime(),
          bufferSize: logs.length,
          activeStreams: 1,
          diskUsage: {
            totalSize: logs.length * 200, // Estimación
            fileCount: 1,
            formattedSize: `${Math.round(logs.length * 200 / 1024)} KB`
          }
        }
      ...
      // Tasks API (for scheduler page)
      if (pathname === '/api/tasks' && method === 'GET') {
        if (!isAuthorizedSoft(req, url, panelDb)) return json(res, 401, { error: 'No autorizado' })
        
        try {
          const { default: taskScheduler } = await import('./task-scheduler.js')
          const tasks = taskScheduler.getAllTasks()
          return json(res, 200, { tasks })
        } catch (error) {
          // Fallback con datos simulados si el módulo no existe
          const mockTasks = [
            {
              id: 1,
              name: 'Backup Diario',
          ...
          return json(res, 200, { tasks: mockTasks })
        }
      }
      ...
      if (pathname === '/api/tasks/executions' && method === 'GET') {
        ...
        try {
          const { default: taskScheduler } = await import('./task-scheduler.js')
          const executions = taskScheduler.getExecutionHistory()
          return json(res, 200, { executions })
        } catch (error) {
          ...
          const mockExecutions = [
            {
              ...
              status: 'completed',
              duration: 300000,
              output: 'Backup completado exitosamente'
            },
            {
              id: 2,
              taskId: 2,
              ...
              status: 'completed',
              duration: 300000,
              output: 'Logs limpiados: 150 archivos eliminados'
            }
          ]
          return json(res, 200, { executions: mockExecutions })
        }
      }
