  ...
  panelServer = http.createServer(async (req, res) => {
    try {
      ...
      if (pathname === '/api/plugins' && method === 'GET') {
        ...
        try {
          const { default: pluginSystem } = await import('./plugin-system.js')
      ...
      if (pathname === '/api/plugins/marketplace' && method === 'GET') {
        ...
        try {
          const { default: pluginSystem } = await import('./plugin-system.js')
          const marketplace = pluginSystem.getMarketplace()
      ...
      if (pathname === '/api/plugins/install' && method === 'POST') {
        ...
        const body = await readJson(req).catch(() => ({}))
        const { pluginId, source = 'marketplace' } = body
        
        if (!pluginId) {
          return json(res, 400, { error: 'Plugin ID es requerido' })
        }
        
        try {
          const { default: pluginSystem } = await import('./plugin-system.js')
          const plugin = await pluginSystem.installPlugin(pluginId, source)
          ...
          return json(res, 201, { success: true, plugin })
        } catch (error) {
          return json(res, 400, { error: error.message })
        }
      }
      ...
      if (pathname.startsWith('/api/plugins/') && pathname.endsWith('/activate') && method === 'POST') {
        ...
        try {
          const { default: pluginSystem } = await import('./plugin-system.js')
          const plugin = await pluginSystem.activatePlugin(pluginId)
          ...
          return json(res, 200, { success: true, plugin })
        } catch (error) {
          return json(res, 400, { error: error.message })
        }
      }

      if (pathname.startsWith('/api/plugins/') && pathname.endsWith('/deactivate') && method === 'POST') {
        if (!isAuthorizedSoft(req, url, panelDb)) return json(res, 401, { error: 'No autorizado' })
        ...
        try {
          const { default: pluginSystem } = await import('./plugin-system.js')
          const plugin = await pluginSystem.deactivatePlugin(pluginId)
      ...
      if (pathname.startsWith('/api/plugins/') && pathname.endsWith('/reload') && method === 'POST') {
        if (!isAuthorizedSoft(req, url, panelDb)) return json(res, 401, { error: 'No autorizado' })
      ...
      if (pathname.startsWith('/api/plugins/') && method === 'DELETE') {
        ...
        try {
          ...
          return json(res, 200, { success: true, message: 'Plugin desinstalado correctamente' })
        } catch (error) {
          return json(res, 400, { error: error.message })
        }
      }

      if (pathname === '/api/plugins/stats' && method === 'GET') {
        if (!isAuthorizedSoft(req, url, panelDb)) return json(res, 401, { error: 'No autorizado' })
      ...
      // Plugin System - Lista de plugins
      if (pathname === '/api/plugins' && method === 'GET') {
        if (!isAuthorizedSoft(req, url, panelDb)) return json(res, 401, { error: 'No autorizado' })
      ...
      // Plugin System - Desactivar plugin
      if (pathname.startsWith('/api/plugins/') && pathname.endsWith('/deactivate') && method === 'POST') {
        if (!isAuthorizedSoft(req, url, panelDb)) return json(res, 401, { error: 'No autorizado' })
      ...
      // Plugin System - Desinstalar plugin
      if (pathname.startsWith('/api/plugins/') && pathname.endsWith('/uninstall') && method === 'DELETE') {
        if (!isAuthorizedSoft(req, url, panelDb)) return json(res, 401, { error: 'No autorizado' })
      ...
      if (pathname === '/api/plugins/marketplace' && method === 'GET') {
        ...
        try {
          const { default: pluginSystem } = await import('./plugin-system.js')
          const marketplace = pluginSystem.getMarketplace()
          const installedPlugins = new Set(pluginSystem.plugins.keys())
          
          // Marcar plugins ya instalados
          const marketplaceWithStatus = marketplace.map(plugin => ({
            ...plugin,
            installed: installedPlugins.has(plugin.id),
            canInstall: !installedPlugins.has(plugin.id)
          }))
          
          return json(res, 200, {
            plugins: marketplaceWithStatus,
            count: marketplace.length,
            categories: [...new Set(marketplace.map(p => p.category || 'general'))]
          })
        } catch (error) {
          return json(res, 500, { error: error.message })
        }
      ...
      // Plugin System - Información específica de plugin
      if (pathname.startsWith('/api/plugins/') && !pathname.includes('/') && method === 'GET') {
        if (!isAuthorizedSoft(req, url, panelDb)) return json(res, 401, { error: 'No autorizado' })
        ...
        try {
          ...
          if (!plugin) {
            return json(res, 404, { error: 'Plugin no encontrado' })
          }
          
          return json(res, 200, {
            ...plugin,
            isActive: pluginSystem.activePlugins.has(pluginId),
            storage: pluginSystem.getPluginStorage(pluginId)
          })
        } catch (error) {
          return json(res, 500, { error: error.message })
        }
      ...
      // Plugin System - Ejecutar hook
      if (pathname === '/api/plugins/hooks/execute' && method === 'POST') {
        if (!isAuthorizedSoft(req, url, panelDb)) return json(res, 401, { error: 'No autorizado' })
        
        const body = await readJson(req).catch(() => ({}))
        const { hookName, args = [] } = body
        
        if (!hookName) {
          return json(res, 400, { error: 'Hook name es requerido' })
        }
        
        try {
          const { default: pluginSystem } = await import('./plugin-system.js')
          const results = await pluginSystem.executeHook(hookName, ...args)
