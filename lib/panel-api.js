...
Panel: ${link}`,
      ...
      if (pathname === '/api/broadcast' && method === 'POST') {
        ...
        try {
          ...
          // Función corregida para detectar tipo de chat
          const detectChatType = (jid) => {
            if (!jid) return null
            if (jid.includes('@newsletter')) return 'channel'   // CORREGIDO: Newsletter son Canales
            if (jid.includes('@broadcast')) return 'broadcast'  // Listas de difusión antiguas
            if (jid.includes('@g.us')) return 'group'          // Grupos y Comunidades
            return null
          }
          ...
          if (sendToAll) {
            ...
            for (const group of allGroups) {
              const jid = group.wa_jid || group.jid
              const type = detectChatType(jid)
              if (jid && type) {
                jids.push(jid)
                // Clasificación simple para estadísticas
            ...
            // Procesar Grupos y Comunidades (ambos usan @g.us)
            if (targetObj.groups === true || targetObj.communities === true) {
              for (const group of allGroups) {
                const jid = group.wa_jid || group.jid
                // Nota: Sin un flag 'isCommunity' en la DB, no podemos distinguir 100%,
                // así que enviamos a todos los @g.us si se selecciona groups o communities.
                if (detectChatType(jid) === 'group') {
                   jids.push(jid)
                   stats.groups++
                }
              }
