...
function normalizeBotJid(raw) {
  ...
  if (!id) return ''
  // Baileys puede incluir sufijo de dispositivo: "12345:17@s.whatsapp.net"
...
async function syncGroups(panelDb) {
  ...
  if (global.conn && global.conn.chats) {
    const newsletters = await Promise.all(Object.values(global.conn.chats).filter(c => 
      c && c.id && (c.id.includes('@newsletter') || c.id.includes('@broadcast'))
    ).map(async c => {
      let name = c.name || c.subject || c.id;
      
      // Intentar obtener metadatos reales si el nombre es solo el ID
      if (name === c.id && c.id.includes('@newsletter') && typeof global.conn.newsletterMetadata === 'function') {
        try {
          const meta = await global.conn.newsletterMetadata('jid', c.id).catch(() => null);
  ...
  for (const group of groups) {
    ...
    // Detectar si es una comunidad basándose en metadata (isCommunity, linkedParent, isCommunityAnnounce)
    const isCommunity = Boolean(group.isCommunity || group.linkedParent || group.isCommunityAnnounce || group.metadata?.isCommunity || group.metadata?.linkedParentJid)
