...
Panel: ${link}`,
      ...
      // Multimedia upload route
      if (pathname === '/api/multimedia' && method === 'POST') {
        const auth = await getJwtAuth(req)
        if (!auth.ok) return json(res, auth.status, { error: auth.error })
      ...
      if (pathname === '/api/multimedia' && method === 'GET') {
        ...
        const limit = Math.min(100, Math.max(1, parseInt(urlParams.get('limit') || '20')))
        const search = safeString(urlParams.get('search') || '')
        const type = safeString(urlParams.get('type') || '')
