...
Panel: ${link}`,
      ...
      if (pathname === '/api/broadcast' && method === 'POST') {
        ...
        try {
          const body = await readJson(req)
          const { message, targets, type } = body // targets: { groups: boolean, channels: boolean, communities: boolean, specific: string[] }
          
          if (!message) return json(res, 400, { error: 'El mensaje es requerido' })
          ...
          // Recopilar JIDs según los objetivos seleccionados
          if (targets.groups) {
            const groups = Object.values(panelDb.groups || {})
            jids.push(...groups.map(g => g.jid))
          }
          
          if (targets.specific && Array.isArray(targets.specific)) {
            jids.push(...targets.specific)
          }
          ...
          (async () => {
            ...
            emitNotification({
              ...
              message: `Se enviaron ${successCount} mensajes exitosamente. Fallidos: ${failCount}.`,
              category: 'system'
            })
          })()
