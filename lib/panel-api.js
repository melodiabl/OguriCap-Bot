...
async function getParticipatingGroups() {
  ...
  console.log('getParticipatingGroups - sock exists:', !!sock)
  console.log('getParticipatingGroups - groupFetchAllParticipating exists:', !!(sock && typeof sock.groupFetchAllParticipating === 'function'))
  
  if (!sock || typeof sock.groupFetchAllParticipating !== 'function') {
    console.log('getParticipatingGroups - No valid connection or method not available')
    return []
  }
  ...
  try {
    ...
    console.log(`getParticipatingGroups - Found ${filtered.length} groups`)
    return filtered
  } catch (error) {
...
async function syncGroups(panelDb) {
  console.log('syncGroups - Starting group synchronization...')
  const groups = await getParticipatingGroups().catch((err) => {
    console.error('syncGroups - Error getting participating groups:', err)
    return []
  })
  
  console.log(`syncGroups - Processing ${groups.length} groups`)
  const now = new Date().toISOString()
  ...
  for (const group of groups) {
    ...
    if (!panelDb.groups[jid]) {
      ...
      console.log(`syncGroups - Added new group: ${subject} (${jid})`)
    } else {
      ...
      console.log(`syncGroups - Updated existing group: ${subject} (${jid})`)
    }
  }

  const totalGroups = Object.keys(panelDb.groups || {}).length
  console.log(`syncGroups - Synchronization complete. Total groups in DB: ${totalGroups}`)
  ...
  panelServer = http.createServer(async (req, res) => {
    try {
      ...
      if (pathname === '/api/grupos/available' && method === 'GET') {
        ...
        console.log('Syncing groups for /api/grupos/available...')
        if (panelDb) await syncGroups(panelDb).catch((err) => {
        ...
        console.log(`Returning ${groups.length} groups from /api/grupos/available`)
        return json(res, 200, { 
          grupos: groups,
          debug: {
      ...
      // Debug endpoint para verificar estado de conexión
      if (pathname === '/api/grupos/debug' && method === 'GET') {
        if (!isAuthorizedSoft(req, url, panelDb)) return json(res, 401, { error: 'No autorizado' })
        
        const sock = global.conn
        const hasConnection = !!sock
        const hasGroupMethod = !!(sock && typeof sock.groupFetchAllParticipating === 'function')
        const connectionStatus = global.stopped || 'unknown'
        
        let groupsFromWA = []
        try {
          if (hasConnection && hasGroupMethod) {
            groupsFromWA = await getParticipatingGroups()
          }
        } catch (err) {
          console.error('Debug - Error getting groups from WA:', err)
        }
        
        const groupsInDB = Object.keys(panelDb?.groups || {}).length
        ...
        return json(res, 200, {
          connection: {
            ...
            hasGroupMethod,
            connectionStatus,
            userJid: sock?.user?.jid || null
          },
          groups: {
            fromWhatsApp: groupsFromWA.length,
            inDatabase: groupsInDB,
            sampleFromWA: groupsFromWA.slice(0, 3).map(g => ({
              jid: g.id || g.jid,
      ...
      if (pathname === '/api/grupos/sync' && method === 'POST') {
        ...
        try {
          ...
          if (clearOld) {
            console.log('Limpiando grupos antiguos...')
            panelDb.groups = {}
            panelDb.groupsCounter = 0
          }
          
          // Sincronizar grupos actuales desde WhatsApp
          console.log('Sincronizando grupos desde WhatsApp...')
          ...
          console.log(`Sincronización completada: ${groups.length} grupos`)
      ...
      if (usuarioViewPasswordMatch && method === 'GET') {
        ...
        console.log('View password request for user ID:', id);
        console.log('Available users:', Object.keys(panelDb?.users || {}));
