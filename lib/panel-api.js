...
async function callAiApi(message, model = 'gpt-3.5-turbo', temperature = 0.7, maxTokens = 1000, conversationHistory = []) {
  try {
    ...
    if (openaiKey) {
      const baseUrl = safeString(process.env.OPENAI_BASE_URL || process.env.PANEL_OPENAI_BASE_URL || 'https://api.openai.com/v1').replace(/\/+$/, '')
      const systemPrompt = safeString(process.env.PANEL_AI_SYSTEM_PROMPT || 'Eres un asistente de IA útil. Responde en español, claro y conciso.')
      const maxHistory = clampInt(process.env.PANEL_AI_HISTORY_MESSAGES || 12, { min: 0, max: 40, fallback: 12 })

      const modelMap = {
        'gpt-4': safeString(process.env.PANEL_OPENAI_MODEL_GPT4 || ''),
        'gpt-3.5-turbo': safeString(process.env.PANEL_OPENAI_MODEL_GPT35 || ''),
        'chatgpt': safeString(process.env.PANEL_OPENAI_MODEL_CHATGPT || ''),
      }

      const requested = safeString(model || '').toLowerCase().trim()
      const mapped =
        modelMap[requested] ||
        safeString(process.env.PANEL_OPENAI_MODEL || '') ||
        'gpt-4o-mini'

      const history = Array.isArray(conversationHistory) ? conversationHistory : []
      const messages = [{ role: 'system', content: systemPrompt }]
      const tail = maxHistory > 0 ? history.slice(-maxHistory) : []
      for (const msg of tail) {
        const role = safeString(msg?.role || '').toLowerCase() === 'assistant' ? 'assistant' : 'user'
      ...
      const res = await axios.post(
        `${baseUrl}/chat/completions`,
        {
          ...
          max_tokens: clampInt(maxTokens, { min: 64, max: 4096, fallback: 1000 }),
        },
        {
          timeout: aiTimeoutMs,
          headers: {
      ...
      const content = safeString(res?.data?.choices?.[0]?.message?.content || '').trim()
      if (!content) throw new Error('Respuesta inválida de OpenAI')
      const tokensUsed = Number(res?.data?.usage?.total_tokens) || 0
      return { content, tokens_used: tokensUsed, model: mapped, provider: 'openai' }
    }

    const allowLegacy = !['0', 'false', 'no', 'off'].includes(safeString(process.env.PANEL_AI_LEGACY_APIS || '0').toLowerCase().trim())
    if (!allowLegacy) {
      throw new Error('IA no configurada: define OPENAI_API_KEY (o habilita PANEL_AI_LEGACY_APIS=1).')
    }
    ...
    try {
      ...
      if (pathname === '/api/notificaciones' && method === 'GET') {
        ...
        const filtered = notifications.map(n => ({
          ...
          fecha_creacion: n.fecha_creacion || n.created_at || n.timestamp || new Date().toISOString(),
          ...
          if (type && type !== 'all' && n.tipo !== type) return false
          if (category && category !== 'all' && n.categoria !== category) return false
          if (read && read !== 'all') {
            const wantRead = read === 'read'
            if (n.leida !== wantRead) return false
          }
          return true
        })
