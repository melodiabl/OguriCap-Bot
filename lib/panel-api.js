...
async function syncGroups(panelDb) {
  ...
  if (global.conn && global.conn.chats) {
    const newsletters = Object.values(global.conn.chats).filter(c => 
  ...
  for (const group of groups) {
    ...
    const isCommunity = Boolean(group.isCommunity || group.linkedParent || group.isCommunityAnnounce)
    const isChannel = jid.includes('@newsletter') || jid.includes('@broadcast') || Boolean(group.isChannel)
    ...
    try {
      ...
      if (pathname === '/api/grupos/broadcast-targets' && method === 'GET') {
        ...
        const allGroups = Object.values(panelDb?.groups || {})
          ...
          .map((g) => {
             ...
             return {
                ...
                es_proveedor: Boolean(g?.es_proveedor),
                participantes: Number(g?.participantes || 0) || 0,
                // Si tienes la data, es útil pasar si es comunidad
        ...
        allGroups.forEach(group => {
          // CORRECCIÓN 2: Lógica de clasificación correcta
          if (!group.wa_jid) return // Saltar si no hay JID válido

          if (group.wa_jid.endsWith('@newsletter')) {
            channels.push(group) // Los newsletters son Canales
          } else if (group.wa_jid.endsWith('@broadcast')) {
            broadcasts.push(group) // Listas de difusión
            // Si prefieres incluirlos en canales o grupos, muévelos allí
          } else if (group.wa_jid.endsWith('@g.us')) {
            // CORRECCIÓN 3: Distinción de comunidades (si tienes el flag isCommunity)
            // Si no tienes el flag, todos los @g.us irán a groups
