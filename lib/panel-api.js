...
export async function startPanelApi({ port, host } = {}) {
  ...
  panelServer = http.createServer(async (req, res) => {
    try {
      ...
      // Solicitar QR del bot principal desde el panel (modo bajo demanda)
      if (method === 'POST' && pathname === '/api/bot/main/request-qr') {
        // Solo roles owner/admin pueden ver el QR del bot principal
        if (!isOwnerOrAdmin(req, url, panelDb)) {
          return json(res, 403, {
            error: 'No tienes permisos para solicitar el QR del bot principal',
          })
        }
        ...
        return json(res, 200, {
          ok: true,
          qr: global.panelApiMainQr || null,
          connection: global.stopped || null,
        })
      }
