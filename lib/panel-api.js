...
import { performDeepCleanup } from './system-utils.js'
...
import { initSocketIO, emitBotStatus, emitBotQR, emitBotConnected, emitBotDisconnected, emitSubbotCreated, emitSubbotQR, emitSubbotPairingCode, emitSubbotConnected, emitSubbotDisconnected, emitSubbotDeleted, emitSubbotUpdated, emitSubbotStatus, emitAporteCreated, emitAporteUpdated, emitPedidoCreated, emitPedidoUpdated, emitGrupoUpdated, emitNotification, emitLogEntry } from './socket-io.js'
...
import { getSubbotCapacityInfo } from './subbot-capacity.js'
...
function withCors(req, res) {
  ...
  const addOriginFromUrl = (list, value) => {
    const raw = String(value || '').trim()
    if (!raw) return
    try {
      list.add(new URL(raw).origin)
    } catch { }
  }
  ...
  for (const o of parseCsv(process.env.CORS_ORIGIN || process.env.PANEL_CORS_ORIGIN || '')) {
    allowedOrigins.add(o)
  }
  for (const o of parseCsv(process.env.SOCKET_IO_CORS_ORIGIN || '')) {
    allowedOrigins.add(o)
  }

  addOriginFromUrl(allowedOrigins, process.env.PANEL_URL)
  addOriginFromUrl(allowedOrigins, process.env.NEXT_PUBLIC_API_URL)

  const allowDomainSuffixes = parseCsv(process.env.CORS_ALLOW_DOMAIN_SUFFIXES || '')
  ...
  let isAllowedBySuffix = false
  if (origin && allowDomainSuffixes.length) {
    try {
      const host = new URL(origin).hostname.toLowerCase()
      isAllowedBySuffix = allowDomainSuffixes.some((suffix) => host === suffix || host.endsWith(`.${suffix}`))
    } catch { }
  }

  if (isAllowedOrigin || isAllowedBySuffix || (process.env.NODE_ENV !== 'production' && isDevelopmentTunnel)) {
  ...
  try {
    ...
    if (!username) {
      if (typeof global.sendTemplateNotification === 'function') {
        global.sendTemplateNotification('unauthorized_access', {
          ip: getClientIP(req),
...
function ensurePanelDb() {
  ...
  panel.systemConfig ||= {
    ...
    autoAddAdminIPOnLogin: true, // Auto-agregar IP al loguear (owner/admin)
  ...
  // Backfill keys if systemConfig already existed
  panel.systemConfig.adminIPs ||= []
  if (typeof panel.systemConfig.allowLocalhost === 'undefined') panel.systemConfig.allowLocalhost = true
  if (typeof panel.systemConfig.autoAddAdminIPOnLogin === 'undefined') panel.systemConfig.autoAddAdminIPOnLogin = true
  if (typeof panel.systemConfig.supportNotifyEmailTo === 'undefined') panel.systemConfig.supportNotifyEmailTo = ''
  if (typeof panel.systemConfig.supportNotifyWhatsAppTo === 'undefined') panel.systemConfig.supportNotifyWhatsAppTo = ''
  if (typeof panel.systemConfig.supportNotifyIncludeAdmins === 'undefined') panel.systemConfig.supportNotifyIncludeAdmins = true
  panel.systemConfig.adminIPMeta ||= {}
...
// Mirror de stdout/stderr hacia el panel (terminal con ANSI)
try {
  const { installTerminalMirror } = await import('./terminal-mirror.js')
  installTerminalMirror()
} catch (e) {
  console.warn('Terminal mirror not available:', e?.message || String(e))
}
...
function normalizeClientIP(ip) {
  const s = safeString(ip).trim()
  if (!s) return ''
  let clean = s
  if (clean.includes(',')) clean = clean.split(',')[0].trim()
  if (clean.toLowerCase().startsWith('::ffff:')) clean = clean.slice(7)
  if (/^\d+\.\d+\.\d+\.\d+:\d+$/.test(clean)) clean = clean.split(':')[0]
  if (clean.startsWith('[') && clean.includes(']')) clean = clean.slice(1, clean.indexOf(']'))
  return clean.trim()
}

function touchAdminIPMeta(panelDb, ip, actor) {
  try {
    if (!panelDb?.systemConfig) return
    const ipNorm = normalizeClientIP(ip) || safeString(ip).trim()
    if (!ipNorm) return
    const username = safeString(actor?.username || actor?.user?.username || '').trim() || 'unknown'
    const role = safeString(actor?.rol || actor?.user?.rol || '').trim() || 'unknown'

    panelDb.systemConfig.adminIPMeta ||= {}
    const meta = panelDb.systemConfig.adminIPMeta
    const now = new Date().toISOString()

    meta[ipNorm] ||= { addedBy: [], firstAddedAt: now, lastAddedAt: now, lastAddedBy: username, lastAddedByRole: role }
    const entry = meta[ipNorm]
    entry.firstAddedAt ||= now
    entry.lastAddedAt = now
    entry.lastAddedBy = username
    entry.lastAddedByRole = role
    entry.addedBy ||= []
    if (!entry.addedBy.includes(username)) entry.addedBy.push(username)
  } catch { }
}

function pruneAdminIPs(panelDb, max = 100) {
  try {
    const list = Array.isArray(panelDb?.systemConfig?.adminIPs) ? panelDb.systemConfig.adminIPs : []
    if (list.length <= max) return
    const meta = (panelDb.systemConfig && typeof panelDb.systemConfig === 'object' ? panelDb.systemConfig.adminIPMeta : null) || {}
    const scored = list.map((ip) => {
      const key = normalizeClientIP(ip) || safeString(ip).trim()
      const entry = key ? meta?.[key] : null
      const last = entry && typeof entry === 'object' ? safeString(entry.lastAddedAt || entry.firstAddedAt || '') : ''
      const ts = last ? Date.parse(last) : 0
      return { ip, ts }
    })
    scored.sort((a, b) => b.ts - a.ts)
    panelDb.systemConfig.adminIPs = scored.slice(0, max).map((x) => x.ip)
  } catch { }
}
...
function getClientIP(req) {
  ...
  if (cfConnectingIP) return normalizeClientIP(cfConnectingIP) || cfConnectingIP
  if (cfRealIP) return normalizeClientIP(cfRealIP) || cfRealIP

  if (forwarded) {
    // x-forwarded-for puede contener múltiples IPs separadas por comas
    const ips = forwarded.split(',').map(ip => ip.trim())
    return normalizeClientIP(ips[0]) || ips[0] // La primera IP es la del cliente original
  }

  if (realIP) {
    return normalizeClientIP(realIP) || realIP
  }
  ...
  return normalizeClientIP(remoteAddress) || remoteAddress || 'unknown'
}
...
function isAllowedIP(clientIP, panelDb) {
  ...
  if (config.allowLocalhost !== false) {
    ...
    if (localhostIPs.includes(clientIP) || localhostIPs.includes(ipNorm)) return true
  }

  // Verificar IPs de administradores
  const adminIPs = config.adminIPs || []
  if (adminIPs.includes(clientIP) || (ipNorm && adminIPs.includes(ipNorm))) return true
  return adminIPs.some((ip) => normalizeClientIP(ip) && normalizeClientIP(ip) === ipNorm)
}
...
function makeSubbotCode() {
  // Codigo solo numerico (sin prefijos) para evitar carpetas tipo `sb_*`.
...
function cleanupBrokenSubbotSymlinks() {
  try {
    const root = getJadiRoot()
    ...
    for (const ent of entries) {
      ...
      // Ya no usamos symlinks en Sessions/SubBot. Eliminar todos.
      try {
...
function deriveSessionDirBaseFromPushname(pushname) {
...
function findPanelSubbotKeyByDir(panelDb, dirName) {
  try {
    const dir = safeString(dirName).trim()
    if (!dir) return null
    const subbots = panelDb?.subbots && typeof panelDb.subbots === 'object' ? panelDb.subbots : {}
    if (subbots[dir]) return dir
    for (const [k, rec] of Object.entries(subbots)) {
      const sessionDir = safeString(rec?.session_dir || rec?.sessionDir || '').trim()
      const code = safeString(rec?.codigo || rec?.code || '').trim()
      const aliasDir = safeString(rec?.alias_dir || rec?.aliasDir || '').trim()
      if (sessionDir === dir || code === dir || aliasDir === dir) return k
    }
    return null
  } catch {
    return null
...
function mergeSubbotRecords(target, source) {
  if (!target || typeof target !== 'object' || !source || typeof source !== 'object') return target
  // Prefer target fields, but backfill missing ones.
  for (const [k, v] of Object.entries(source)) {
    if (typeof target[k] === 'undefined' || target[k] === null || target[k] === '') target[k] = v
  }
  return target
}

function normalizeSubbotPhoneDigits(value) {
  return safeString(value || '').replace(/[^0-9]/g, '')
}

function isRealSessionDir(dirName) {
  try {
    const dir = safeString(dirName).trim()
    if (!dir) return false
    const full = path.join(getJadiRoot(), dir)
...
function sessionDirHasCreds(dirName) {
  try {
    const dir = safeString(dirName).trim()
    if (!dir) return false
    const credsPath = path.join(getJadiRoot(), dir, 'creds.json')
...
function chooseBestSubbotRecordKey(phone, items) {
  const digits = normalizeSubbotPhoneDigits(phone)
  if (!digits || !Array.isArray(items) || items.length === 0) return null
  // Prefer canonical key
  const direct = items.find((it) => it && it.key === digits)
  if (direct) return digits

  // Prefer a record pointing to a real session dir with creds
  for (const it of items) {
    const rec = it?.rec
    const dir = safeString(rec?.session_dir || rec?.sessionDir || '').trim()
    if (dir && isRealSessionDir(dir) && sessionDirHasCreds(dir)) return it.key
  }
  // Prefer a record pointing to a real session dir
  for (const it of items) {
    const rec = it?.rec
    const dir = safeString(rec?.session_dir || rec?.sessionDir || '').trim()
    if (dir && isRealSessionDir(dir)) return it.key
  }
...
function dedupePanelSubbots(panelDb) {
  try {
    if (!panelDb?.subbots || typeof panelDb.subbots !== 'object') return { removed: 0 }
    const subbots = panelDb.subbots
    const groups = new Map()

    for (const [key, rec] of Object.entries(subbots)) {
      const phone = normalizeSubbotPhoneDigits(rec?.numero) || normalizeSubbotPhoneDigits(key)
      if (!phone) continue
      if (!groups.has(phone)) groups.set(phone, [])
      groups.get(phone).push({ key, rec })
    }
    ...
    for (const [phone, items] of groups.entries()) {
      ...
      const keepKey = chooseBestSubbotRecordKey(phone, items)
      if (!keepKey) continue

      const canonicalKey = normalizeSubbotPhoneDigits(phone)
      if (!canonicalKey) continue

      // Move/ensure canonical record under the phone digits
      if (!subbots[canonicalKey] && subbots[keepKey]) {
        subbots[canonicalKey] = subbots[keepKey]
      }
      const keepRec = subbots[canonicalKey]
      if (!keepRec || typeof keepRec !== 'object') continue

      // Prefer session_dir that exists on disk (pushname dir)
      if (!isRealSessionDir(keepRec.session_dir)) {
        const bestDirItem = items.find((it) => {
          const dir = safeString(it?.rec?.session_dir || it?.rec?.sessionDir || '').trim()
          return dir && isRealSessionDir(dir)
        })
        if (bestDirItem) {
          keepRec.session_dir = safeString(bestDirItem.rec.session_dir || bestDirItem.rec.sessionDir || '').trim()
          keepRec.alias_dir = keepRec.session_dir
        }
      }
      ...
      for (const it of items) {
        ...
        if (!subbots[k]) continue
        if (subbots[k] === keepRec) {
          try { delete subbots[k] } catch { }
          removed++
          continue
        }
        try { mergeSubbotRecords(keepRec, subbots[k]) } catch { }
        try { delete subbots[k] } catch { }
        removed++
      }

      // Normalize identifiers
      keepRec.numero = canonicalKey
      keepRec.code = canonicalKey
      keepRec.codigo = canonicalKey
      if (!keepRec.id) keepRec.id = canonicalKey
      if (!keepRec.alias_dir && keepRec.session_dir) keepRec.alias_dir = keepRec.session_dir
    }
...
function normalizeSubbotSessionDirs(panelDb, { dryRun = false } = {}) {
  const root = getJadiRoot()
  ...
  for (const dirName of sessionDirs) {
    const summary = readSubbotCredsSummary(dirName)
    const phone = safeString(summary?.phone || '').trim()
    if (!phone) {
      report.skipped_no_phone++
      continue
    }
    const name = safeString(summary?.name || '').trim()
    const baseFromName = deriveSessionDirBaseFromPushname(name)
    if (!baseFromName) {
      report.skipped_no_name++
      continue
    }

    const currentPath = path.join(root, dirName)
    let dirNameUsed = dirName

    // Elegir nombre final por pushname (con sufijos para evitar colisiones)
    const last4 = phone ? String(phone).slice(-4) : ''
    const preferSuffix = baseFromName.length <= 2
    const desiredCandidates = (preferSuffix
      ? [
        last4 ? `${baseFromName}_${last4}` : null,
        phone ? `${baseFromName}_${phone}` : null,
        baseFromName,
        `${baseFromName}_${dirName}`,
      ]
      : [
        baseFromName,
        last4 ? `${baseFromName}_${last4}` : null,
    ...
    let desired = null
    for (const candRaw of desiredCandidates) {
      ...
      const desiredPathTry = path.join(root, cand)
      try {
        const st = fs.lstatSync(desiredPathTry)
        if (st?.isSymbolicLink?.()) {
          // No usamos symlinks: eliminarlos y continuar.
          if (!dryRun) {
            try { fs.rmSync(desiredPathTry, { recursive: false, force: true }) } catch { }
          }
          desired = cand
          break
        }
        // Existe y no es symlink -> conflicto
        continue
      } catch {
        // no existe
        desired = cand
    ...
    const needsRename = Boolean(desired && desired !== dirName)
    if (needsRename) {
      const sock = findConnBySubbotCode(phone) || findConnBySubbotCode(dirName)
      if (sock) {
        const rs =
          ...
          null
        const active = typeof rs === 'number' ? rs !== 3 : Boolean(sock?.user || sock?.isInit)
        if (active) {
          report.skipped_online++
          desired = null
        }
      }

      if (desired) {
        const desiredPath = path.join(root, desired)
        try {
          if (!dryRun) fs.renameSync(currentPath, desiredPath)
          report.migrated++
          dirNameUsed = desired
        } catch {
          report.conflicts++
    ...
    // Actualizar record en panelDb (key estable por numero) y session_dir
    try {
      const stableKey = phone
      const oldKey = findPanelSubbotKeyByDir(panelDb, dirName)
      const source = oldKey ? panelDb.subbots?.[oldKey] : null
      if (source) {
        if (!panelDb.subbots[stableKey] || panelDb.subbots[stableKey] === source) {
          panelDb.subbots[stableKey] = source
        } else {
          mergeSubbotRecords(panelDb.subbots[stableKey], source)
        }

        const rec = panelDb.subbots[stableKey]
        rec.id = rec.id || source.id || nextSubbotId(panelDb)
        rec.code = stableKey
        rec.codigo = stableKey
        rec.numero = stableKey
        rec.nombre_whatsapp = rec.nombre_whatsapp || name || null
        rec.session_dir = dirNameUsed
        rec.alias_dir = dirNameUsed
        if ((!rec.usuario || safeString(rec.usuario).trim() === 'auto') && stableKey) {
          const u = resolvePanelUsernameByWhatsApp(panelDb, stableKey)
          if (u) rec.usuario = u
        }
        rec.updated_at = new Date().toISOString()
        if (oldKey && oldKey !== stableKey) delete panelDb.subbots[oldKey]
...
function normalizeBotJid(raw) {
  const id = safeString(raw || '').trim()
  if (!id) return ''

  // Extraer la parte del número/ID base sin sufijos de dispositivo ni dominio
  // Formatos posibles: "12345:17@s.whatsapp.net", "12345@s.whatsapp.net", "12345"
  let base = id.split('@')[0]
...
function getCurrentBotJid() {
  return normalizeBotJid(global?.conn?.user?.id || global?.conn?.user?.jid || '')
}
...
async function syncGroups(panelDb) {
  ...
  panelDb.groups ||= {}
  panelDb.meta ||= {}
  panelDb.meta.whatsapp ||= {}

  const currentBotJid = getCurrentBotJid()
  const storedBotJid = normalizeBotJid(panelDb?.meta?.whatsapp?.bot_jid || panelDb?.meta?.whatsapp?.botJid || '')
  // Si cambió el número/cuenta del bot, limpiar cache de grupos para evitar mezclar sesiones
  if (currentBotJid && storedBotJid && storedBotJid !== currentBotJid) {
    panelDb.groups = {}
    panelDb.groupsCounter = 0
  }
  if (currentBotJid) panelDb.meta.whatsapp.bot_jid = currentBotJid

  let groups = await getParticipatingGroups().catch((err) => {
    return []
  })

  // Incluir newsletters (canales) desde conn.chats si existen
  if (global.conn) {
    let newsletters = []

    // Intentar obtener de conn.chats (cache de Baileys/Simple)
    if (global.conn.chats) {
      newsletters = Object.values(global.conn.chats).filter(c =>
        c && c.id && (c.id.includes('@newsletter') || c.id.includes('@broadcast'))
      ).map(c => ({
        id: c.id,
    ...
    // Intentar obtener via API de Baileys si está disponible (GetSubscribedNewsletters)
    if (typeof global.conn.getNewsletters === 'function' && newsletters.length < 5) {
      try {
        const fetched = await global.conn.getNewsletters().catch(() => [])
        if (Array.isArray(fetched) && fetched.length > 0) {
          fetched.forEach(n => {
            if (!newsletters.some(existing => existing.id === n.id)) {
              newsletters.push({
                id: n.id,
                subject: n.subject || n.name || n.id,
                isChannel: true,
                tipo: 'channel'
              })
            }
        ...
        console.warn('syncGroups - Error fetching newsletters via getNewsletters:', err.message)
      }
    }

    // Si aún no hay nada y tenemos newsletterMetadata, intentar buscar en la DB
    if (newsletters.length === 0 && typeof global.conn.newsletterMetadata === 'function') {
      // Intentar recuperar de chats de la DB
      const dbChats = Object.keys(global.db?.data?.chats || {})
      const newsletterJids = dbChats.filter(jid => jid.includes('@newsletter'))

      for (const jid of newsletterJids) {
        try {
          const meta = await global.conn.newsletterMetadata('jid', jid).catch(() => null)
          if (meta) {
            newsletters.push({
              id: jid,
              subject: meta.name || jid,
              isChannel: true,
              tipo: 'channel'
            })
          }
    ...
    // Incluir canales configurados manualmente (settings.js / env)
    // Esto permite que el panel muestre el canal aunque no exista en conn.chats.
    try {
      const configured = []
      if (global.ch && typeof global.ch === 'object') {
        configured.push(...Object.values(global.ch))
      }
      const fromEnv = String(process.env.WHATSAPP_CHANNEL_JIDS || '').trim()
      if (fromEnv) {
        configured.push(...fromEnv.split(/[\s,]+/g))
      }
      ...
      for (const jid of uniq) {
        ...
        if (newsletters.some((existing) => existing.id === jid)) continue

        let subject = jid
        if (typeof global.conn.newsletterMetadata === 'function') {
          const meta = await global.conn.newsletterMetadata('jid', jid).catch(() => null)
          if (meta) subject = meta.name || meta.subject || meta.title || jid
        }
    ...
    if (newsletters.length > 0) {
      const groupIds = new Set(groups.map(g => g.id || g.jid))
      newsletters.forEach(n => {
        if (!groupIds.has(n.id)) groups.push(n)
      })
    }
  }

  // Si no hay grupos ni canales, marcar que se intentó
  if (groups.length === 0) {
  ...
  for (const group of groups) {
    ...
    const participantes =
      Array.isArray(group.participants) ? group.participants.length :
        (typeof group.size === 'number' ? group.size : 0)

    // Detectar si es una comunidad basándose en metadata (isCommunity, isCommunityAnnounce)
    const isCommunity = Boolean(group.isCommunity || group.isCommunityAnnounce || group.metadata?.isCommunity || group.metadata?.isCommunityAnnounce)
    const isChannel = (jid && (jid.includes('@newsletter') || jid.includes('@broadcast'))) || Boolean(group.isChannel)
    const isSubGroup = Boolean(group.linkedParent || group.metadata?.linkedParentJid)

    // Un grupo puede ser "community" si es el anuncio o tiene metadata de comunidad
    // Si es sub-grupo pero no es el anuncio, lo tratamos como grupo normal
    const tipo = group.tipo || (isChannel ? 'channel' : (isCommunity ? 'community' : 'group'))
    ...
    if (!panelDb.groups[jid]) {
      ...
      const prevPart = Number(existing.participantes || 0) || 0
      const prevBotJid = normalizeBotJid(existing.bot_jid || existing.botJid || '')
      if (existing.nombre !== subject || existing.descripcion !== desc || prevPart !== participantes || (currentBotJid && prevBotJid !== currentBotJid)) {
        existing.wa_jid = jid
        existing.bot_jid = currentBotJid || null
        existing.nombre = subject
        if (typeof desc === 'string') existing.descripcion = desc
        existing.participantes = participantes
        existing.isCommunity = isCommunity
  ...
  if (newGroupsCount > 0 || updatedGroupsCount > 0) {
    const totalChannels = Object.values(panelDb.groups).filter(g => g.tipo === 'channel').length
    const totalCommunities = Object.values(panelDb.groups).filter(g => g.tipo === 'community').length
    const totalGroups = Object.values(panelDb.groups).filter(g => g.tipo === 'group').length
    console.log(`syncGroups - Sync completed: ${totalGroups} groups, ${totalChannels} channels, ${totalCommunities} communities. (New: ${newGroupsCount}, Updated: ${updatedGroupsCount})`)
  }

  // Limpiar el rate limit si la sincronización fue exitosa
  // Purga segura: si obtuvimos grupos desde WhatsApp, eliminar cache viejo (incluye conexiones anteriores)
  if (seen.size) {
    for (const [jid, g] of Object.entries(panelDb.groups || {})) {
      const groupBotJid = normalizeBotJid(g?.bot_jid || g?.botJid || '')
      if (currentBotJid && groupBotJid && groupBotJid !== currentBotJid) {
        delete panelDb.groups[jid]
        continue
      }
      if (!seen.has(jid)) delete panelDb.groups[jid]
    }
  }
...
function getSubbotSessionInfo(record) {
  const root = getJadiRoot()
  const code = safeString(record?.session_dir || record?.sessionDir || record?.codigo || record?.code || '').trim()
  const aliasDir = safeString(record?.alias_dir || record?.aliasDir || '').trim()

  const candidates = [code, aliasDir]
    .map((s) => safeString(s).trim())
    .filter(Boolean)

  let hasSessionDir = false
  let hasCreds = false
  let sessionDirUsed = null
  ...
  for (const dir of candidates) {
    ...
    if (!sessionDirUsed) sessionDirUsed = dir
  }

  return { hasSessionDir, hasCreds, sessionDirUsed }
}

// Lee config.json opcional de un subbot (Sessions/SubBot/<dir>/config.json)
const __subbotCfgCache = new Map()
function readSubbotConfig(sessionDir) {
  try {
    const dir = safeString(sessionDir).trim()
    if (!dir) return null

    const now = Date.now()
    const cached = __subbotCfgCache.get(dir)
    if (cached && (now - cached.ts) < 10 * 1000) return cached.value

    const cfgPath = path.join(getJadiRoot(), dir, 'config.json')
    if (!fs.existsSync(cfgPath)) {
      __subbotCfgCache.set(dir, { ts: now, value: null })
      return null
    }
    const raw = fs.readFileSync(cfgPath, 'utf8')
    const parsed = raw ? JSON.parse(raw) : null
    const name = safeString(parsed?.name || '').trim().slice(0, 64) || null
    const prefix = safeString(parsed?.prefix || '').trim().slice(0, 32) || null
    const banner = safeString(parsed?.banner || '').trim().slice(0, 512) || null
    const video = safeString(parsed?.video || '').trim().slice(0, 512) || null
    const value = { name, prefix, banner, video }
    __subbotCfgCache.set(dir, { ts: now, value })
    return value
  } catch {
    return null
  }
...
// Lee un resumen ligero desde creds.json (numero/nombre) para mostrar subbots offline
const __subbotCredsCache = new Map()
function readSubbotCredsSummary(sessionDir) {
  try {
    const dir = safeString(sessionDir).trim()
    if (!dir) return null

    const now = Date.now()
    const cached = __subbotCredsCache.get(dir)
    if (cached && (now - cached.ts) < 10 * 1000) return cached.value

    const credsPath = path.join(getJadiRoot(), dir, 'creds.json')
    if (!fs.existsSync(credsPath)) {
      __subbotCredsCache.set(dir, { ts: now, value: null })
      return null
    }
    ...
    const jid = safeString(me?.id || me?.jid || '').trim()
    let base = jid ? jid.split('@')[0] : ''
    if (base.includes(':')) base = base.split(':')[0]
    const phone = base ? base.replace(/[^0-9]/g, '') : ''
    const name = safeString(me?.name || parsed?.name || '').trim().slice(0, 80)
    ...
    __subbotCredsCache.set(dir, { ts: now, value })
    return value
  } catch {
    return null
  }
...
function resolvePanelUsernameByWhatsApp(panelDb, phone) {
  try {
    const digits = normalizeWhatsAppNumber(phone)
    if (!digits) return null
    const users = Object.values(panelDb?.users || {})
    const found = users.find((u) => normalizeWhatsAppNumber(u?.whatsapp_number || u?.whatsapp || u?.phone) === digits)
    const username = safeString(found?.username || '').trim()
...
function invalidateSubbotConfigCache(sessionDir) {
  try {
    const dir = safeString(sessionDir).trim()
    if (!dir) return
    __subbotCfgCache.delete(dir)
  } catch { }
}

function writeSubbotConfig(sessionDir, patch) {
  const dir = safeString(sessionDir).trim()
  if (!dir) return { ok: false, error: 'sessionDir requerido' }
  try {
    const cfgPath = path.join(getJadiRoot(), dir, 'config.json')
    let current = {}
    if (fs.existsSync(cfgPath)) {
    ...
    if (patch && typeof patch === 'object') {
      if ('name' in patch) next.name = safeString(patch.name).trim().slice(0, 64)
      if ('prefix' in patch) next.prefix = safeString(patch.prefix).trim().slice(0, 32)
      if ('banner' in patch) next.banner = safeString(patch.banner).trim().slice(0, 512)
      if ('video' in patch) next.video = safeString(patch.video).trim().slice(0, 512)
    }
    fs.writeFileSync(cfgPath, JSON.stringify(next, null, 2), 'utf8')
    invalidateSubbotConfigCache(dir)
    return { ok: true }
  } catch (e) {
...
function normalizePrefixRuntime(prefix) {
  const raw = safeString(prefix).trim()
  if (!raw) return null
...
function normalizeSubbotForPanel(record, { isOnline }) {
  ...
  const isError = rawEstado === 'error'
  const estado = isOnline ? 'activo' : (isError ? 'error' : 'inactivo')
  ...
  const pairingCodeRaw = record.pairingCode || record.pairing_code || null
  const qrDataRaw = record.qr_data || record.qrCode || record.qr_code || null

  const sessionInfo = getSubbotSessionInfo(record)
  const subbotCfg = sessionInfo.sessionDirUsed ? readSubbotConfig(sessionInfo.sessionDirUsed) : null
  const customName = subbotCfg?.name || null
  const customPrefix = subbotCfg?.prefix || null
  const customBanner = subbotCfg?.banner || null
  const customVideo = subbotCfg?.video || null
  const parseTs = (v) => {
  ...
  const createdTs = parseTs(record.fecha_creacion || record.created_at)
  const updatedTs = Math.max(
    createdTs,
    parseTs(record.ultima_actividad || record.updated_at || record.fecha_actualizacion || record.fecha_actualizacion)
  )
  const nowTs = Date.now()
  const withinPendingWindow = updatedTs > 0 ? (nowTs - updatedTs) <= 10 * 60 * 1000 : false
  const hasPendingAuth = Boolean(
    sessionInfo.hasSessionDir &&
    !sessionInfo.hasCreds &&
    ((qrDataRaw || pairingCodeRaw) || withinPendingWindow)
  )
  ...
  const pairingCode = connectionState === 'needs_auth' ? pairingCodeRaw : null
  const qrData = connectionState === 'needs_auth' ? qrDataRaw : null

  const createdBy = record.created_by || record.createdBy || null
  const createdByRole = record.created_by_role || record.createdByRole || null
  const createdFrom = record.created_from || record.createdFrom || null
  const owner = record.owner || null
  const createdByJid = record.created_by_jid || record.createdByJid || null
  ...
  return {
    ...
    connectionState,
    hasCreds: sessionInfo.hasCreds,
    hasSessionDir: sessionInfo.hasSessionDir,
    sessionDir: sessionInfo.sessionDirUsed,

    // Config opcional por subbot (config.json)
    ...
    customVideo,
    displayName: customName || nombreWhatsapp || aliasDir || numero || codigo || null,
  }
}
...
function buildWhatsAppSendCandidates(rawNumber) {
  const cleaned = normalizeWhatsAppNumber(rawNumber)
  if (!cleaned) return []

  const candidates = [cleaned]
  const withoutLeadingZero = cleaned.replace(/^0+/, '')
  if (withoutLeadingZero && withoutLeadingZero !== cleaned) candidates.push(withoutLeadingZero)
...
async function sendWhatsAppText(number, text) {
  try {
    const candidates = buildWhatsAppSendCandidates(number)
    if (!candidates.length) return { ok: false, reason: 'invalid_number', tried: [] }
    if (!global.conn?.sendMessage || !global.conn?.user) return { ok: false, reason: 'bot_not_connected', tried: candidates }

    for (const candidate of candidates) {
      try {
        await global.conn.sendMessage(`${candidate}@s.whatsapp.net`, { text: safeString(text || '') })
        return {
          ok: true,
          number: candidate,
          tried: candidates,
          normalizedFrom: normalizeWhatsAppNumber(number),
        }
      } catch { }
    }
    ...
    return { ok: false, reason: 'send_exception', tried: buildWhatsAppSendCandidates(number) }
  }
}
...
async function deliverCredentialsToUser(user, { password, reason }) {
  ...
  return { delivered: null, deliveredTo: null, reason: 'no_delivery_channel_or_send_failed' }
}
...
async function notifyPedidoOwner(pedido, { status, actor, note } = {}) {
  ...
  if (wa) {
    const waResult = await sendWhatsAppText(wa, message)
    if (waResult?.ok) return { delivered: 'whatsapp', deliveredTo: maskWhatsAppNumber(waResult.number || wa) }
  }
...
async function autoCleanupDisconnectedSubbots() {
  try {
    if (typeof global.loadDatabase === 'function') await global.loadDatabase()
    const panelDb = ensurePanelDb()
    if (!panelDb?.subbots) return { cleaned: 0 }

    const root = getJadiRoot()
    if (!fs.existsSync(root)) return { cleaned: 0 }

    let cleaned = 0
    const recordsToDelete = []

    for (const [code, rec] of Object.entries(panelDb.subbots)) {
      const sessionDir = rec.session_dir || rec.sessionDir || code
      const fullPath = path.join(root, sessionDir)
      const credsPath = path.join(fullPath, 'creds.json')

      // 1) Verificar si existe carpeta y tiene creds.json válido
      let hasValidCreds = false
      const dirExists = fs.existsSync(fullPath)
      try {
        if (dirExists && fs.existsSync(credsPath)) {
          const credsData = JSON.parse(fs.readFileSync(credsPath, 'utf8'))
          hasValidCreds = Boolean(credsData?.me || credsData?.authState?.creds?.me)
        }
      } catch { }

      // Si no tiene carpeta, o no tiene creds válido y no está online, marcar para eliminar
      const sock = findConnBySubbotCode(code)
      const isOnline = Boolean(sock && isSockOnline(sock))

      if ((!dirExists || !hasValidCreds) && !isOnline) {
        recordsToDelete.push(code)
    ...
    // Eliminar records de la DB y emitir evento
    for (const code of recordsToDelete) {
      delete panelDb.subbots[code]
      emitSubbotDeleted(code)
    }

    if (cleaned > 0) {
      try { if (global.db?.write) void global.db.write() } catch { }
    }

    return { cleaned, codes: recordsToDelete }
  } catch (error) {
    return { cleaned: 0, error: String(error?.message || error) }
  }
}
...
async function getSubbotsList() {
  ...
  // AUTO-LIMPIEZA: eliminar subbots desconectados sin sesión válida
  await autoCleanupDisconnectedSubbots()

  const sessionDirs = listSessionDirs()
  for (const dirName of sessionDirs) {
    const fullPath = path.join(getJadiRoot(), dirName)

    // IGNORAR carpetas marcadas como eliminadas (archivo .deleted presente)
    const deletedMarkerPath = path.join(fullPath, '.deleted')
    if (fs.existsSync(deletedMarkerPath)) {
      continue
    }
    ...
    const credsSummary = readSubbotCredsSummary(dirName)
    const phone = credsSummary?.phone || null
    const ownerUsername = phone ? resolvePanelUsernameByWhatsApp(panelDb, phone) : null

    const stableKey = safeString(phone || '').trim() || dirName
    const existingKey = findPanelSubbotKeyByDir(panelDb, dirName)
    const source = existingKey ? panelDb.subbots?.[existingKey] : null
    const target = panelDb.subbots?.[stableKey] || null

    if (source && (!target || target === source)) {
      panelDb.subbots[stableKey] = source
      if (existingKey && existingKey !== stableKey) delete panelDb.subbots[existingKey]
    } else if (source && target && target !== source) {
      mergeSubbotRecords(target, source)
      if (existingKey && existingKey !== stableKey) delete panelDb.subbots[existingKey]
    }

    panelDb.subbots[stableKey] ||= { id: nextSubbotId(panelDb) }
    const rec = panelDb.subbots[stableKey]
    rec.code = stableKey
    rec.codigo = stableKey
    rec.tipo = rec.tipo || 'qr'
    const currentUsuario = safeString(rec.usuario || '').trim()
    rec.usuario = (!currentUsuario || currentUsuario === 'auto') ? (ownerUsername || 'auto') : currentUsuario
    rec.numero = rec.numero || phone
    rec.nombre_whatsapp = rec.nombre_whatsapp || credsSummary?.name || null
    rec.fecha_creacion = rec.fecha_creacion || (stats.birthtime || stats.mtime || new Date()).toISOString()
    rec.session_dir = dirName
  }

  // Limpiar duplicados (ej: claves sb_* antiguas) y dejar 1 registro por numero
  try {
    const { removed } = dedupePanelSubbots(panelDb)
    if ((removed || 0) > 0) {
      try { if (global.db?.write) void global.db.write() } catch { }
    }
  } catch { }
  ...
  const result = records
    .map((rec) => {
      ...
      const sock =
        (code ? findConnBySubbotCode(code) : null) ||
        (rec?.numero ? findConnBySubbotCode(rec.numero) : null) ||
        (rec?.alias_dir ? findConnBySubbotCode(rec.alias_dir) : null)
      // En algunos forks `sock.user` puede quedar undefined aunque la conexión esté abierta.
      const isOnline = Boolean(sock && isSockOnline(sock))
      const sessionInfo = getSubbotSessionInfo(rec)

      // Completar datos básicos desde creds.json cuando el subbot está offline
      try {
        const summary = sessionInfo.sessionDirUsed ? readSubbotCredsSummary(sessionInfo.sessionDirUsed) : null
        if (summary) {
          if (!rec.numero && summary.phone) rec.numero = summary.phone
          if (!rec.nombre_whatsapp && summary.name) rec.nombre_whatsapp = summary.name
          if ((!rec.usuario || safeString(rec.usuario).trim() === 'auto') && summary.phone) {
            const u = resolvePanelUsernameByWhatsApp(panelDb, summary.phone)
            if (u) rec.usuario = u
          }
        }
      } catch { }

      // Si ya existe sesión (creds), cualquier QR/pairing guardado es stale: limpiarlo para evitar "Esperando pairing/QR" falso
      if ((sessionInfo.hasCreds || !sessionInfo.hasSessionDir) && (rec.qr_data || rec.qrCode || rec.qr_code || rec.pairingCode || rec.pairing_code)) {
        rec.qr_data = null
        rec.qrCode = null
        rec.qr_code = null
        rec.pairingCode = null
        rec.pairing_code = null
        try {
          if (global.db?.write) void global.db.write()
        } catch { }
      }
      if (isOnline) {
        ...
        if (jid) {
          ...
          rec.numero = rec.numero || phone
          if ((!rec.usuario || safeString(rec.usuario).trim() === 'auto') && phone) {
            const u = resolvePanelUsernameByWhatsApp(panelDb, phone)
            if (u) rec.usuario = u
          }
        }
...
function resolveSubbotRecordByParam(panelDb, idOrCode) {
  ...
  if (found) return found
  // Buscar por session_dir o alias_dir si no se encontró por código/id
  const foundByDir = Object.values(panelDb.subbots).find((r) => {
    const sessionDir = r.session_dir || r.sessionDir || ''
    const aliasDir = r.alias_dir || r.aliasDir || ''
    return sessionDir === param || aliasDir === param
  })
  return foundByDir || null
}
...
async function deleteSubbotByCode(code) {
  ...
  // Si no hay record en DB, intentar eliminar directamente del disco como subbot huérfano
  if (!record) {
    const orphanResult = await deleteOrphanSubbotFromDisk(code)
  ...
  if (sessionDir) {
    ...
    try {
      // Crear archivo .deleted PRIMERO para marcar como eliminado (evita que reaparezca)
      const deletedMarkerPath = path.join(sessionPath, '.deleted')
      fs.writeFileSync(deletedMarkerPath, JSON.stringify({ deletedAt: new Date().toISOString(), reason: 'user_deleted' }))
      ...
      // Si no se puede eliminar la carpeta, al menos quedó marcada con .deleted
      // y no aparecerá en el panel
    }
  }
  ...
  try {
    ...
    if (phone) {
      ...
      // NO usar existsSync aquí: si el symlink está roto, existsSync() retorna false.
...
// Eliminar subbot huérfano directamente del disco (sin record en DB)
async function deleteOrphanSubbotFromDisk(code) {
  try {
    const root = getJadiRoot()
    if (!fs.existsSync(root)) return { success: false, error: 'Directorio de sesiones no existe' }

    // Buscar directorio exacto o por número/código
    const targetDir = String(code || '').trim()
    if (!targetDir) return { success: false, error: 'Código inválido' }

    const sessionPath = path.join(root, targetDir)
    ...
    if (fs.existsSync(sessionPath)) {
      ...
      if (stat.isDirectory()) {
        fs.rmSync(sessionPath, { recursive: true, force: true })
        return { success: true, removed: targetDir }
      }
    }

    // Buscar en subdirectorios comunes
    const possiblePaths = [
      path.join(root, 'SubBot', targetDir),
      path.join(root, 'SubBots', targetDir),
      path.join(root, 'subbots', targetDir),
      path.join(root, targetDir.replace(/[^0-9]/g, '')) // Versión solo números
    ]
    ...
    for (const p of possiblePaths) {
      if (fs.existsSync(p)) {
        ...
        if (stat.isDirectory()) {
          ...
          return { success: true, removed: p }
        }
      }
    }

    return { success: false, error: 'Directorio no encontrado en disco' }
  } catch (error) {
    return { success: false, error: String(error?.message || error) }
  }
}

// Limpiar subbots huérfanos (carpetas en disco sin record en DB o sin creds.json válido)
async function cleanupOrphanSubbots() {
  try {
    if (typeof global.loadDatabase === 'function') await global.loadDatabase()
    const panelDb = ensurePanelDb()
    if (!panelDb) return { cleaned: 0, errors: [], details: [] }

    const root = getJadiRoot()
    if (!fs.existsSync(root)) return { cleaned: 0, errors: [], details: [] }
    ...
    for (const dirName of sessionDirs) {
      try {
        ...
        // Verificar si está marcado como eliminado
        const isMarkedDeleted = fs.existsSync(deletedMarkerPath)

        // Verificar si tiene record en DB
        const existingKey = findPanelSubbotKeyByDir(panelDb, dirName)
        const hasRecord = Boolean(existingKey && panelDb.subbots?.[existingKey])

        // Verificar si tiene creds.json válido
        let hasValidCreds = false
        try {
          if (fs.existsSync(credsPath)) {
            const credsData = JSON.parse(fs.readFileSync(credsPath, 'utf8'))
            // Creds válido si tiene me o authState
            hasValidCreds = Boolean(credsData?.me || credsData?.authState?.creds?.me)
          }
        } catch { }

        // Es huérfano si: está marcado como eliminado, no tiene record en DB, O no tiene creds válido
        const isOrphan = isMarkedDeleted || !hasRecord || !hasValidCreds

        if (isOrphan) {
          // Desconectar si está online
          const sock = findConnBySubbotCode(dirName)
          if (sock) {
            try { await sock.logout() } catch { }
            const idx = global.conns?.indexOf(sock)
            if (typeof idx === 'number' && idx >= 0) global.conns.splice(idx, 1)
          ...
          // Eliminar directorio físicamente
          fs.rmSync(fullPath, { recursive: true, force: true })
          cleaned++

          let reason = 'orphan'
          if (isMarkedDeleted) reason = 'marked_deleted'
          else if (!hasRecord) reason = 'no_db_record'
          else if (!hasValidCreds) reason = 'no_valid_creds'

          results.push({ dir: dirName, reason, removed: true })

          // Eliminar de DB si existe
          if (existingKey && panelDb.subbots?.[existingKey]) {
            delete panelDb.subbots[existingKey]
          }
        }
      } catch (err) {
        errors.push({ dir: dirName, error: String(err?.message || err) })
      }
    }

    return { cleaned, errors, details: results }
  } catch (error) {
    return { cleaned: 0, errors: [String(error?.message || error)], details: [] }
  }
}
...
async function callAiApi(message, model = 'gpt-3.5-turbo', temperature = 0.7, maxTokens = 1000, conversationHistory = []) {
  try {
    ...
    const aiTimeoutMs = clampInt(
      process.env.PANEL_AI_TIMEOUT_MS || process.env.AI_CHAT_TIMEOUT_MS || 12000,
      { min: 1000, max: 60000, fallback: 12000 }
    )

    const allowLegacy = !['0', 'false', 'no', 'off'].includes(safeString(process.env.PANEL_AI_LEGACY_APIS || '1').toLowerCase().trim())
    if (!allowLegacy) {
      throw new Error('IA deshabilitada (PANEL_AI_LEGACY_APIS=0).')
    }
    ...
    switch (apiProvider) {
      case 'delirius':
        ...
        const deliriusRes = await axios.get(deliriusUrl, { timeout: aiTimeoutMs })
      ...
      case 'zenzxz':
        ...
        const zenzRes = await axios.get(zenzUrl, { timeout: aiTimeoutMs })
      ...
      case 'adonix':
        ...
        const adonixRes = await axios.get(adonixUrl, { timeout: aiTimeoutMs })
    ...
    const errMsg = safeString(error?.message || '').trim()
    const fallbackContent = errMsg && /ia no configurada/i.test(errMsg) ? errMsg : randomFallback
    ...
    try {
      ...
      if (method === 'GET' && pathname.startsWith('/media/')) {
        ...
        if (!fs.existsSync(filePath) || !fs.statSync(filePath).isFile()) {
          // Si el archivo no existe, verificar si está registrado en la base de datos multimedia
          // y eliminar la entrada para mantener la consistencia
          const fileName = path.basename(filePath);

          // Buscar en la base de datos multimedia si existe un registro con este nombre de archivo
          if (panelDb?.multimedia) {
            for (const [id, item] of Object.entries(panelDb.multimedia)) {
              if (item && typeof item === 'object' && item.filename === fileName) {
                // Eliminar el registro huérfano de la base de datos
                console.warn(`Archivo multimedia huérfano detectado y eliminado: ${fileName}`);
          ...
          console.log(`Archivo no encontrado: ${filePath} (mediaRoot: ${mediaRoot})`);
          console.log(`Ruta solicitada: ${pathname}, Archivo relativo: ${rel}`);
      ...
      if (method === 'GET' && pathname === '/api/health') {
        const clientIP = getClientIP(req)
        const token = getTokenFromRequest(req, url)
        const user = token ? getUserFromToken(token, panelDb) : null
        const role = safeString(user?.rol || '').toLowerCase()
        const ipAllowed = isAllowedIP(clientIP, panelDb)
        const isAdmin = ['owner', 'admin', 'administrador'].includes(role)

        const turnstileEnabled = String(process.env.TURNSTILE_DISABLED || '').trim() !== '1'
        const turnstileSiteKey = turnstileEnabled
          ? (safeString(process.env.TURNSTILE_SITE_KEY || process.env.NEXT_PUBLIC_TURNSTILE_SITE_KEY).trim() || null)
          : null
        ...
        return json(res, 200, {
          ...
          canAccessDuringMaintenance: ipAllowed || isAdmin,
          ipAllowed,
          role: role || null,
      ...
      if (pathname.startsWith('/api/auth/') && (method === 'POST' || method === 'GET')) {
        try {
          ...
          // Crear usuario admin inicial SOLO si esta configurado por env.
          // No se crean credenciales por defecto en el repo.
          const envAdminUser = safeString(process.env.PANEL_ADMIN_USER || 'admin').trim() || 'admin'
          const envAdminPass = safeString(process.env.PANEL_ADMIN_PASS).trim()
          const envAdminRole = safeString(process.env.PANEL_ADMIN_ROLE || 'owner').trim() || 'owner'
          const bcryptRounds = clampInt(process.env.BCRYPT_ROUNDS, { min: 4, max: 15, fallback: 10 })

          if (Object.keys(db.data.usuarios).length === 0) {
            if (envAdminPass) {
              const adminPassword = await bcrypt.hash(envAdminPass, bcryptRounds)
              db.data.usuarios[1] = {
                id: 1,
              ...
              console.log(`✅ Usuario admin inicial creado: ${envAdminUser} (${envAdminRole})`)
            } else {
              if (!global.__panelAdminInitWarned) {
                global.__panelAdminInitWarned = true
                console.warn('No se creo admin inicial: configura PANEL_ADMIN_PASS en el entorno')
              }
            }
          }
          ...
          const getConfig = () => ({
            security: {
              jwtSecret: safeString(process.env.JWT_SECRET || '').trim(),
              jwtExpiry: safeString(process.env.JWT_EXPIRY || '24h').trim() || '24h',
              bcryptRounds,
            },
          })
          ...
          if (pathname === '/api/auth/login' && method === 'POST') {
            ...
            const ipAllowed = isAllowedIP(clientIp, panelDb)
            const turnstileEnabled = String(process.env.TURNSTILE_DISABLED || '').trim() !== '1'
            const allowBypassOnAllowedIp = String(process.env.TURNSTILE_BYPASS_ALLOWED_IPS || '').trim() === '1'
            const bypass = Boolean(!turnstileEnabled || (allowBypassOnAllowedIp && ipAllowed))

            if (turnstileEnabled && !bypass) {
              if (!turnstileToken) {
                return json(res, 400, { error: 'Token de verificación Turnstile requerido' });
              }

              const turnstileSecretKey = safeString(process.env.TURNSTILE_SECRET_KEY).trim()
              if (!turnstileSecretKey) {
                return json(res, 500, { error: 'TURNSTILE_SECRET_KEY no configurada' })
              }
              try {
                ...
                if (!turnstileData.success) {
                  return json(res, 401, {
                    error: 'Verificación de Turnstile fallida. Intenta de nuevo.',
                    errorCodes: turnstileData['error-codes'] || turnstileData.error_codes || null,
                  });
                }
              } catch (turnstileError) {
                console.error('Error validando Turnstile:', turnstileError);
                return json(res, 500, { error: 'Error al validar verificación de seguridad' });
              }
            }
            ...
            // Responder primero para evitar timeouts del frontend.
            // Persistencia/notificaciones se realizan en segundo plano.
            const payload = {
              token,
              user: {
                id: user.id,
            ...
            // Auto-agregar IP del admin/owner si está habilitado
            try {
              const panelDbLogin = ensurePanelDb()
              const roleLower = safeString(user.rol || '').toLowerCase()
              const isAdminRole = ['owner', 'admin', 'administrador'].includes(roleLower)
              const ipToAdd = normalizeClientIP(clientIp) || clientIp

              if (
                panelDbLogin?.systemConfig?.autoAddAdminIPOnLogin &&
                isAdminRole &&
                ipToAdd &&
                ipToAdd !== 'unknown'
              ) {
                const adminIPs = Array.isArray(panelDbLogin.systemConfig?.adminIPs)
                  ? panelDbLogin.systemConfig.adminIPs
                  : []
                const exists = adminIPs.some((ip) => normalizeClientIP(ip) === ipToAdd)
                if (!exists) {
                  panelDbLogin.systemConfig.adminIPs = [...adminIPs, ipToAdd]
                }
                touchAdminIPMeta(panelDbLogin, ipToAdd, { username: user.username, rol: user.rol })
                pruneAdminIPs(panelDbLogin, 100)
              }
            } catch { }
            ...
            setImmediate(async () => {
              try {
                ...
                console.error('[AUTH] post-login tasks failed:', e)
              }
            })
          ...
          if (pathname === '/api/auth/register-public' && method === 'POST') {
            ...
            // Validación profunda de existencia de correo real (MX, SMTP, Typos)
            try {
              const emailValidator = (await import('deep-email-validator')).default
              const validationResult = await emailValidator(emailStr)
              if (!validationResult.valid && validationResult.reason !== 'smtp') {
                // Si falla en cualquier cosa excepto SMTP puro (a veces los SMTP bloquean scripts), lo consideramos inválido
                return json(res, 400, { 
                  error: 'El correo proporcionado no es un correo real o activo', 
                  details: validationResult.validators[validationResult.reason]?.reason || 'Email verification failed'
                })
              }
            } catch (validatorError) {
              console.warn('Fallo en la validación profunda de email:', validatorError)
            }
            ...
            if (db.pool?.query) {
              try {
                const result = await db.pool.query(
                  ...
                  `,
                  [usernameStr, hashed, 'usuario', whatsappClean || null, true, clientIp || null, JSON.stringify(metadata)]
                )
            ...
            try {
              const { sendRegistrationEmail, getEmailServiceStatus } = await import('./email-service.js')
              if (!getEmailServiceStatus()?.configured) {
                warnings.push('No se pudo enviar email de confirmación porque SMTP no está configurado.')
              }
              void sendRegistrationEmail({ to: emailStr, username: usernameStr }).catch(() => { })
            } catch { }

            // Notificación de registro (panel y email admin)
            try {
              if (typeof global.sendTemplateNotification === 'function') {
                global.sendTemplateNotification('user_registered', {
                  username: usernameStr,
                  email: emailStr
                });
              }
          ...
          if (pathname === '/api/auth/change-password' && method === 'POST') {
            ...
            try {
              if (typeof global.sendTemplateNotification === 'function') {
                global.sendTemplateNotification('password_changed', {
          ...
          if (pathname === '/api/auth/reset-password' && method === 'POST') {
            ...
            try {
              if (typeof global.sendTemplateNotification === 'function') {
                global.sendTemplateNotification('password_reset_requested', {
                  username: username
                });
              }
            } catch { }
      ...
      // API de vinculación del bot principal centralizada en index.js
      if (typeof global.handleMainBotBindingApiRoute === 'function') {
        const handledByIndex = await global.handleMainBotBindingApiRoute({
          req,
          res,
          url,
          pathname,
      ...
      if (pathname === '/api/bot/send' && method === 'POST') {
        ...
        try {
          ...
          emitNotification({
            ...
            message: `Tu mensaje fue enviado correctamente a ${jid.split('@')[0]}.`,
            categoria: 'bot'
          })
      ...
      if (pathname === '/api/bot/broadcast' && method === 'POST') {
        ...
        emitNotification({
          type: successCount === jids.length ? 'success' : 'warning',
          title: 'Difusión de Mensaje Completada',
          message: `El mensaje fue enviado exitosamente a ${successCount} de ${jids.length} chat${jids.length !== 1 ? 's' : ''}.`,
          categoria: 'bot'
        })
      ...
      if (pathname === '/api/system/config' && method === 'GET') {
        ...
        const clientIP = getClientIP(req)
        const token = getTokenFromRequest(req, url)
        const currentUser = getUserFromToken(token, panelDb)
        const role = safeString(currentUser?.rol || '').toLowerCase()
        const isOwner = role === 'owner'
        const adminIPs = Array.isArray(panelDb.systemConfig?.adminIPs) ? panelDb.systemConfig.adminIPs : []
        const username = safeString(currentUser?.username || '').trim()
        const meta = (panelDb.systemConfig && typeof panelDb.systemConfig === 'object' ? panelDb.systemConfig.adminIPMeta : null) || {}
        const myAdminIPs = username
          ? adminIPs.filter((ip) => {
            const key = normalizeClientIP(ip) || safeString(ip).trim()
            const entry = key ? meta?.[key] : null
            const addedBy = entry && typeof entry === 'object' ? entry.addedBy : null
            return Array.isArray(addedBy) && addedBy.includes(username)
          })
          : []
        const showAll = isOwner && url?.searchParams?.get('allAdminIPs') === '1'
        const payload = {
          ...(panelDb.systemConfig || {}),
          currentIP: clientIP, // Mostrar IP actual para facilitar configuración
          currentIPAllowed: isAllowedIP(clientIP, panelDb),
          adminIPsCount: adminIPs.length,
          myAdminIPs,
          myAdminIPsCount: myAdminIPs.length,
        }
        // Privacidad: no exponer lista global de IPs por defecto
        delete payload.adminIPs
        delete payload.adminIPMeta
        if (showAll) payload.adminIPs = adminIPs
      ...
      if (pathname === '/api/system/config' && method === 'PATCH') {
        ...
        try {
          ...
          const payload = {
            ...
            currentIPAllowed: isAllowedIP(getClientIP(req), panelDb),
            adminIPsCount: adminIPs.length,
            myAdminIPs,
            myAdminIPsCount: myAdminIPs.length,
          }
      ...
      if (pathname === '/api/system/add-admin-ip' && method === 'POST') {
        ...
        const clientIP = getClientIP(req)
        const norm = normalizeClientIP(clientIP)
        const adminIPs = Array.isArray(panelDb.systemConfig?.adminIPs) ? panelDb.systemConfig.adminIPs : []
        ...
        const ipToAdd = norm || clientIP
        const exists = adminIPs.some((ip) => normalizeClientIP(ip) && normalizeClientIP(ip) === ipToAdd)
        if (!exists) {
          panelDb.systemConfig = panelDb.systemConfig || {}
          panelDb.systemConfig.adminIPs = [...adminIPs, ipToAdd]
          touchAdminIPMeta(panelDb, ipToAdd, currentUser)
          pruneAdminIPs(panelDb, 100)
          console.log(`✅ IP agregada como administrador: ${ipToAdd}`)
          try {
            if (global.db?.write) await global.db.write()
          } catch { }
        } else {
          // aunque ya exista, actualizar metadata para "pertenencia"
          touchAdminIPMeta(panelDb, ipToAdd, currentUser)
          pruneAdminIPs(panelDb, 100)
          try {
            if (global.db?.write) await global.db.write()
        ...
        const adminIPsFinal = Array.isArray(panelDb.systemConfig.adminIPs) ? panelDb.systemConfig.adminIPs : []
        const metaFinal = (panelDb.systemConfig && typeof panelDb.systemConfig === 'object' ? panelDb.systemConfig.adminIPMeta : null) || {}
        const myAdminIPs = username
          ? adminIPsFinal.filter((ip) => {
            const key = normalizeClientIP(ip) || safeString(ip).trim()
            const entry = key ? metaFinal?.[key] : null
            const addedBy = entry && typeof entry === 'object' ? entry.addedBy : null
            return Array.isArray(addedBy) && addedBy.includes(username)
          })
          : []
        ...
        const payload = {
          success: true,
          addedIP: ipToAdd,
          adminIPsCount: adminIPsFinal.length,
          myAdminIPs,
          myAdminIPsCount: myAdminIPs.length,
        }
        return json(res, 200, payload)
      ...
      if (pathname === '/api/config' && method === 'GET') {
        ...
        if (!['owner', 'admin', 'administrador'].includes(role)) return json(res, 403, { error: 'Permisos insuficientes' })
      ...
      if (/^\/api\/config\/stats\/?$/.test(pathname) && method === 'GET') {
        const auth = authenticateAndAuthorize(req, url, panelDb)
        if (!auth.authorized) return json(res, auth.status, { error: auth.error })
      ...
      if (/^\/api\/config\/[^/]+\/?$/.test(pathname) && method === 'GET') {
        const auth = authenticateAndAuthorize(req, url, panelDb)
        if (!auth.authorized) return json(res, auth.status, { error: auth.error })
        ...
        try {
          let configKey = pathname.split('/')[3] || ''
          try { configKey = decodeURIComponent(configKey) } catch { }
          configKey = safeString(configKey).trim()
          if (!/^[a-zA-Z0-9_-]+$/.test(configKey) || configKey === 'stats') {
            return json(res, 400, { error: 'Invalid config key' })
          }
          const { default: configManager } = await import('./config-manager.js')
      ...
      if (/^\/api\/config\/[^/]+\/?$/.test(pathname) && method === 'PUT') {
        const auth = authenticateAndAuthorize(req, url, panelDb)
        if (!auth.authorized) return json(res, auth.status, { error: auth.error })
        ...
        try {
          ...
          if (/validaci/i.test(msg)) {
            const raw = msg.includes(':') ? msg.split(':').slice(1).join(':') : ''
            const validationErrors = raw
              .split(',')
              .map((s) => s.trim())
              .filter(Boolean)
            return json(res, 400, { error: 'Validation failed', validationErrors })
          }
      ...
      if (pathname.startsWith('/api/config/') && pathname.endsWith('/rollback') && method === 'POST') {
        ...
        try {
          ...
          const body = await readJson(req).catch(() => ({}))
          const versionId = safeString(body?.versionId || '').trim()
          if (!versionId) return json(res, 400, { error: 'Missing versionId' })
          const { default: configManager } = await import('./config-manager.js')
      ...
      if (pathname === '/api/email/status' && method === 'GET') {
        ...
        try {
          const { getEmailServiceStatus } = await import('./email-service.js')
          return json(res, 200, getEmailServiceStatus())
        } catch {
          return json(res, 500, { error: 'Error loading email status' })
        }
      }
      ...
      if (pathname === '/api/email/verify' && method === 'POST') {
        ...
        try {
          const { verifySmtp } = await import('./email-service.js')
          const result = await verifySmtp()
          return json(res, 200, result)
        } catch {
          return json(res, 500, { error: 'Error verifying SMTP' })
        }
      }
      ...
      if (pathname === '/api/email/test' && method === 'POST') {
        ...
        try {
          const body = await readJson(req).catch(() => ({}))
          const to = safeString(body?.to || '').trim()
          const { sendTestEmail } = await import('./email-service.js')
          const result = await sendTestEmail({ to: to || null })
          ...
          if (result?.skipped) {
            return json(res, 200, { ok: false, skipped: true, reason: result.reason })
          }

          return json(res, 400, { ok: false, reason: result?.reason || 'Error al enviar el email de prueba. Verifique la configuración SMTP.' })
        } catch (error) {
          return json(res, 500, { error: error?.message || 'Error sending test email' })
        }
      }
      ...
      if (pathname === '/api/email/preview' && method === 'GET') {
        ...
        try {
          const template = safeString(url.searchParams.get('template') || 'test').trim() || 'test'
          const { getEmailTemplatePreview, buildBroadcastPreview } = await import('./email-service.js')

          // Handle broadcast templates
          if (template.startsWith('broadcast_')) {
            const broadcastType = template.replace('broadcast_', '')
            return json(res, 200, buildBroadcastPreview(broadcastType))
          }

          return json(res, 200, getEmailTemplatePreview(template))
        } catch (error) {
          return json(res, 500, { error: error?.message || 'Error building email preview' })
        }
      }
      ...
      if (pathname === '/api/bot/global-state' && method === 'POST') {
        ...
        try {
          ...
          emitNotification({
            ...
            message: isOn ? 'El bot de WhatsApp ha sido activado y está operativo en todos los grupos.' : 'El bot de WhatsApp ha sido desactivado y no responderá en ningún grupo.',
            categoria: 'bot'
          })
        } catch { }
      ...
      if (pathname === '/api/bot/global-shutdown' && method === 'POST') {
        ...
        emitNotification({
          type: 'warning',
          title: 'Apagado Global del Bot',
          message: mensaje || 'El bot ha sido desactivado globalmente por el administrador y no responderá mensajes.',
          categoria: 'sistema'
        })
      ...
      if (pathname === '/api/grupos' && method === 'GET') {
        // Verificar si se solicita específicamente para broadcast
        const forBroadcast = url.searchParams.get('forBroadcast') === 'true'

        // Grupos endpoint necesita datos actualizados pero no en cada request
        if (panelDb) await syncGroupsOnDemand(panelDb).catch(() => { })

        if (forBroadcast) {
          // Modo para broadcast: clasificar en grupos, canales y comunidades
          const currentBotJid = getCurrentBotJid()
          panelDb.meta ||= {}
          panelDb.meta.whatsapp ||= {}
          const storedBotJid = normalizeBotJid(panelDb?.meta?.whatsapp?.bot_jid || panelDb?.meta?.whatsapp?.botJid || '')
          const hasAnyGroups = panelDb?.groups && Object.keys(panelDb.groups).length > 0
          const hasMixedBotJid =
            hasAnyGroups && currentBotJid
              ? Object.values(panelDb.groups).some((g) => {
                const bj = normalizeBotJid(g?.bot_jid || g?.botJid || '')
                return bj && bj !== currentBotJid
              })
              : false
          const hasMissingBotJid =
            hasAnyGroups && currentBotJid
              ? Object.values(panelDb.groups).some((g) => !normalizeBotJid(g?.bot_jid || g?.botJid || ''))
              : false

          const shouldResync =
            Boolean(global?.conn?.user) &&
            Boolean(currentBotJid) &&
            (!storedBotJid || storedBotJid !== currentBotJid || hasMixedBotJid || hasMissingBotJid)

          if (shouldResync) await syncGroups(panelDb).catch(() => { })
          else await syncGroupsOnDemand(panelDb).catch(() => { })

          const activeBotJid =
            normalizeBotJid(panelDb?.meta?.whatsapp?.bot_jid || panelDb?.meta?.whatsapp?.botJid || '') || getCurrentBotJid()

          const allGroups = Object.values(panelDb?.groups || {})
            .filter((g) => {
              if (!activeBotJid) return true
              const gBotJid = normalizeBotJid(g?.bot_jid || g?.botJid || '')
              // Si el grupo no tiene bot_jid asignado, permitirlo (probablemente de una sync antigua o universal)
              if (!gBotJid) return true
              return gBotJid === activeBotJid
            })
            .map((g) => {
              const jid = g?.wa_jid || g?.jid
              ...
              let tipo = g?.tipo
              if (!tipo) {
                if (jid && jid.includes('@newsletter')) tipo = 'channel'
                else if (jid && jid.includes('@broadcast')) tipo = 'channel'
                else if (g?.isCommunity || (jid && jid.includes('@g.us') && g?.isCommunity)) tipo = 'community'
                else tipo = 'group'
              }
              ...
              return {
                ...
                participantes: Number(g?.participantes || 0),
                tipo: tipo,
                isCommunity: Boolean(g?.isCommunity),
                isChannel: Boolean(g?.isChannel),
          ...
          // Clasificar usando el campo tipo ya mapeado
          const groups = allGroups.filter(g => g.tipo === 'group')
          const channels = allGroups.filter(g => g.tipo === 'channel')
          const communities = allGroups.filter(g => g.tipo === 'community')
          ...
          return json(res, 200, {
            ...
            debug: {
              ...
              botJid: activeBotJid || null,
            }
          })
        } else {
          // Modo normal con filtros y paginación
          ...
          return json(res, 200, { items, pagination })
        }
      }
      ...
      if (pathname === '/api/grupos' && method === 'POST') {
        ...
        if (!panelDb.groups[jid]) {
          ...
          if (!panelDb.groups[jid].bot_jid) panelDb.groups[jid].bot_jid = getCurrentBotJid() || null
      ...
      if (pathname === '/api/grupos/available' && method === 'GET') {
        ...
        // Evitar mezclar grupos de conexiones anteriores (otro número/cuenta)
        try {
          const currentBotJid = getCurrentBotJid()
          panelDb.meta ||= {}
          panelDb.meta.whatsapp ||= {}
        ...
        const groups = Object.values(panelDb?.groups || {})
          .filter((g) => {
            if (!activeBotJid) return true
            return normalizeBotJid(g?.bot_jid || g?.botJid || '') === activeBotJid
          })
          .map((g) => ({
            id: g?.id || 0,
            jid: g?.wa_jid || g?.jid,
            ...
            descripcion: safeString(g?.descripcion || ''),
            es_proveedor: Boolean(g?.es_proveedor),
            participantes: Number(g?.participantes || 0) || 0,
          }))
      ...
      // Grupos, canales y comunidades disponibles para broadcast
      if (pathname === '/api/grupos/broadcast-targets' && method === 'GET') {
        if (!isAuthorizedSoft(req, url, panelDb)) return json(res, 401, { error: 'No autorizado' })
        if (!panelDb) return json(res, 500, { error: 'DB no disponible' })

        // Sincronizar grupos si es necesario
        try {
          const currentBotJid = getCurrentBotJid()
          panelDb.meta ||= {}
          panelDb.meta.whatsapp ||= {}
          ...
          // Verificar si hay mezcla de JIDs o faltantes
          const groupsList = Object.values(panelDb.groups || {})
          const hasMixedBotJid = hasAnyGroups && currentBotJid
            ? groupsList.some((g) => {
              const bj = normalizeBotJid(g?.bot_jid || g?.botJid || '')
              return bj && bj !== currentBotJid
            })
          ...
          const hasMissingBotJid = hasAnyGroups && currentBotJid
            ? groupsList.some((g) => !normalizeBotJid(g?.bot_jid || g?.botJid || ''))
            : false
          ...
          console.error('Error sincronizando grupos:', e)
        }

        const activeBotJid = normalizeBotJid(panelDb?.meta?.whatsapp?.bot_jid || panelDb?.meta?.whatsapp?.botJid || '') || getCurrentBotJid()

        // Filtrar y Mapear de forma segura
        const allGroups = Object.values(panelDb?.groups || {})
          .filter((g) => {
            if (!activeBotJid) return true
            return normalizeBotJid(g?.bot_jid || g?.botJid || '') === activeBotJid
          ...
          .map((g) => {
            // CORRECCIÓN 1: Asegurar string con || '' para evitar crash
            const jid = g?.wa_jid || g?.jid || ''

            let tipo = g?.tipo
            if (!tipo) {
              if (jid.includes('@newsletter')) tipo = 'channel'
              else if (jid.includes('@broadcast')) tipo = 'channel'
            ...
            return {
              ...
              participantes: Number(g?.participantes || 0) || 0,
              tipo: tipo,
              isCommunity: g?.isCommunity || g?.isParent || false,
              isChannel: tipo === 'channel'
            }
          })

        // Clasificar según el tipo de JID
        const groups = []
        const channels = []
        const communities = []
        // Opcional: listas de difusión legacy
        const broadcasts = []

        allGroups.forEach(group => {
          if (!group.wa_jid) return

          if (group.tipo === 'channel') {
            channels.push(group)
          } else if (group.tipo === 'community') {
        ...
        return json(res, 200, {
          ...
          channels,
          communities,
          broadcasts, // Puedes devolver esto también si es útil
          total: allGroups.length,
          debug: {
            totalGroups: groups.length,
            totalChannels: channels.length,
      ...
      if (pathname === '/api/grupos/sync' && method === 'POST') {
        ...
        try {
          ...
          try {
            emitNotification({
              type: 'success',
              title: 'Sincronización de Grupos Completada',
              message: `Se sincronizaron ${groups.length} grupo${groups.length !== 1 ? 's' : ''} de WhatsApp exitosamente${clearOld ? '. Los grupos antiguos fueron eliminados' : ''}.`,
              categoria: 'bot'
            })
          } catch { }
      ...
      if (groupToggleMatch && method === 'POST') {
        ...
        // Emitir evento Socket.IO
        const groupName = panelDb?.groups?.[groupId]?.nombre || 'el grupo seleccionado'
        ...
        emitNotification({
          ...
          message: `El bot ha sido ${enable ? 'activado' : 'desactivado'} en "${groupName}".`,
          categoria: 'bot'
        })
      ...
      if (pathname === '/api/subbots/status' && method === 'GET') {
        ...
        const subbots = list.map((s) => ({ subbotId: s.code, isOnline: s.isOnline, status: s.status, connectionState: s.connectionState }))
        return json(res, 200, { subbots })
      }

      // Subbots reindex (descubrir sesiones en disco)
      if (pathname === '/api/subbots/reindex' && method === 'POST') {
        if (typeof global.loadDatabase === 'function') await global.loadDatabase()
        const panelDb = ensurePanelDb()
        const auth = authenticateAndAuthorize(req, url, panelDb)
        if (!auth.authorized) return json(res, auth.status, { error: auth.error })
        const role = safeString(auth.user?.rol || 'usuario').toLowerCase()
        if (!['owner', 'admin', 'administrador'].includes(role)) return json(res, 403, { error: 'Permisos insuficientes' })

        const removedSymlinks = (() => {
          try { return cleanupBrokenSubbotSymlinks()?.removed || 0 } catch { return 0 }
        })()
        const list = await getSubbotsList()
        try { emitSubbotStatus() } catch { }
        return json(res, 200, { success: true, count: list.length, removed_symlinks: removedSymlinks })
      }

      // Subbots normalize (renombrar carpetas a numero de WhatsApp)
      if (pathname === '/api/subbots/normalize' && method === 'POST') {
        const body = await readJson(req).catch(() => ({}))
        ...
        const report = normalizeSubbotSessionDirs(panelDb, { dryRun })
        try { if (global.db?.write && !dryRun) void global.db.write() } catch { }
        try { emitSubbotStatus() } catch { }
      ...
      if (pathname === '/api/subbots/qr' && method === 'POST') {
        ...
        let code = String(id)
        while (panelDb.subbots?.[code] || hasEntry(path.join(root, code))) {
        ...
        const record = {
          ...
          usuario,
          owner: usuario,
          created_by: safeString(auth.user?.username || '').trim() || null,
          created_by_role: safeString(auth.user?.rol || '').trim() || null,
          created_from: 'panel',
        ...
        // Notificar creación de subbot
        if (typeof global.sendTemplateNotification === 'function') {
          global.sendTemplateNotification('subbot_created', {
            subbotCode: code
          });
        }
      ...
      if (pathname === '/api/subbots/code' && method === 'POST') {
        ...
        const code = numero
        if (panelDb?.subbots?.[code]) return json(res, 409, { error: 'Ya existe un subbot con ese numero' })

        const root = getJadiRoot()
        const sessionPath = path.join(root, code)
        // Si existe un symlink roto con el mismo nombre, eliminarlo para poder crear la carpeta.
        try {
          ...
          if (st.isSymbolicLink() && !fs.existsSync(sessionPath)) {
            ...
            return json(res, 409, { error: 'Ya existe una sesion con ese numero' })
          }
        } catch { }
        fs.mkdirSync(sessionPath, { recursive: true })
      ...
      if (delMatch && method === 'DELETE') {
        ...
        // Notificar eliminación de subbot
        if (typeof global.sendTemplateNotification === 'function') {
      ...
      if (pathname === '/api/subbot/reindex' && method === 'POST') {
        if (typeof global.loadDatabase === 'function') await global.loadDatabase()
        const panelDb = ensurePanelDb()
        const auth = authenticateAndAuthorize(req, url, panelDb)
        if (!auth.authorized) return json(res, auth.status, { success: false, error: auth.error })
        const role = safeString(auth.user?.rol || 'usuario').toLowerCase()
        if (!['owner', 'admin', 'administrador'].includes(role)) return json(res, 403, { success: false, error: 'Permisos insuficientes' })
      ...
      if (pathname === '/api/subbot/normalize' && method === 'POST') {
        const body = await readJson(req).catch(() => ({}))
        const dryRun = Boolean(body?.dryRun || body?.dry_run)
        if (typeof global.loadDatabase === 'function') await global.loadDatabase()
        const panelDb = ensurePanelDb()
      ...
      if (pathname === '/api/subbot/capacity' && method === 'GET') {
        ...
        const cap = getSubbotCapacityInfo({
          hardMax: cfg.hardMax ?? cfg.hard_max,
          autoLimit: cfg.autoLimit ?? cfg.auto_limit,
          maxSubbots: cfg.maxSubbots ?? cfg.max_subbots,
          reserveMB: cfg.reserveMB ?? cfg.reserve_mb,
          perBotMB: cfg.perBotMB ?? cfg.per_bot_mb,
        })
        return json(res, 200, { success: true, capacity: cap })
      }
      if (pathname === '/api/subbot/create' && method === 'POST') {
        ...
        // Capacidad (limitar creacion masiva desde panel)
        try {
          const cfg = global.db?.data?.panel?.whatsapp?.subbots || {}
          const cap = getSubbotCapacityInfo({
            hardMax: cfg.hardMax ?? cfg.hard_max,
          ...
          // Limitar por SUBBOTS CONECTADOS (no por subbots creados en disco)
          if ((cap.connectedSubs || 0) >= cap.effectiveMax) {
            return json(res, 429, { success: false, error: `Límite de SubBots alcanzado (${cap.effectiveMax}).` })
          }
        } catch { }
        ...
        let id = 0
        if (type === 'code') {
          code = String(phoneNumber || '').replace(/[^0-9]/g, '')
          if (!code) return json(res, 400, { success: false, error: 'phoneNumber es requerido' })
          if (panelDbLocal?.subbots?.[code]) return json(res, 409, { success: false, error: 'Ya existe un subbot con ese numero' })

          const sessionPathExisting = path.join(root, code)
          try {
            const st = fs.lstatSync(sessionPathExisting)
            if (st.isSymbolicLink() && !fs.existsSync(sessionPathExisting)) {
              fs.rmSync(sessionPathExisting, { recursive: false, force: true })
            } else {
              return json(res, 409, { success: false, error: 'Ya existe una sesion con ese numero' })
            }
          } catch { }
          id = nextSubbotId(panelDbLocal)
        } else {
          id = nextSubbotId(panelDbLocal)
          code = String(id)
          while (panelDbLocal?.subbots?.[code] || hasEntry(path.join(root, code))) {
            id = nextSubbotId(panelDbLocal)
            code = String(id)
          }
        }
      ...
      if (pathname === '/api/subbot/settings' && method === 'POST') {
        ...
        const body = await readJson(req).catch(() => ({}))
        const subbotId = body?.subbotId || body?.code
        if (!subbotId) return json(res, 400, { success: false, error: 'subbotId es requerido' })

        const record = resolveSubbotRecordByParam(panelDbLocal, subbotId)
        if (!record) return json(res, 404, { success: false, error: 'Subbot no encontrado' })

        // Verificar permisos: Solo el dueño del subbot o un admin puede editarlo
        const viewerRole = safeString(auth.user?.rol || 'usuario').toLowerCase()
        const isAdmin = ['owner', 'admin', 'administrador'].includes(viewerRole)
        if (!isAdmin && safeString(record.usuario) !== safeString(auth.user?.username)) {
          return json(res, 403, { success: false, error: 'No tienes permiso para editar este subbot' })
        }

        const updates = {}
        const sock = findConnBySubbotCode(record.codigo || record.code)
        const online = isSockOnline(sock)
        const sessionInfo = getSubbotSessionInfo(record)
        const sessionDirUsed = sessionInfo.sessionDirUsed

        // 1. Alias (local)
        if (typeof body.alias === 'string') {
          record.alias_dir = body.alias.trim()
          updates.alias = record.alias_dir
        }
        ...
        if (typeof body.name === 'string' && body.name.trim()) {
          ...
          if (online) {
            try {
              ...
              record.nombre_whatsapp = newName
              updates.name = newName
            } catch (err) {
              console.error(`Error actualizando nombre de subbot ${subbotId}:`, err)
            }
          } else {
            record.nombre_whatsapp = newName
            updates.name = newName
        ...
        // 3. Bio/Estado (WhatsApp)
        if (typeof body.status === 'string') {
          const newStatus = body.status.trim()
          if (online) {
            try {
              await sock.updateProfileStatus(newStatus)
              record.whatsapp_status = newStatus
              updates.status = newStatus
            } catch (err) {
              console.error(`Error actualizando estado de subbot ${subbotId}:`, err)
            }
          } else {
            record.whatsapp_status = newStatus
            updates.status = newStatus
        ...
        if (body.pfp) {
          try {
            let buffer
            if (body.pfp.startsWith('data:image')) {
              const base64Data = body.pfp.split(',')[1]
              buffer = Buffer.from(base64Data, 'base64')
            } else {
              buffer = Buffer.from(body.pfp, 'base64')
            }

            if (online) {
              const image = await Jimp.read(buffer)
              const resized = await jimpGetBuffer(image, MIME_JPEG)
              await sock.updateProfilePicture(sock.user.jid, resized)
              updates.pfp = 'updated'
            } else {
              // Si no está online, no podemos actualizar la foto en WA, 
              // pero podríamos guardarla temporalmente si fuera necesario.
              return json(res, 400, { success: false, error: 'El subbot debe estar conectado para cambiar la foto de perfil' })
            }
          } catch (err) {
            console.error(`Error actualizando foto de subbot ${subbotId}:`, err)
            return json(res, 500, { success: false, error: 'Error al procesar la imagen' })
          }
        }

        // 5. Identidad del SubBot (menu/prefix/banner/video) - config.json
        const hasIdentityPatch =
          typeof body.customName === 'string' || typeof body.custom_name === 'string' ||
          typeof body.customPrefix === 'string' || typeof body.custom_prefix === 'string' ||
          typeof body.customBanner === 'string' || typeof body.custom_banner === 'string' ||
          typeof body.customVideo === 'string' || typeof body.custom_video === 'string'

        if (hasIdentityPatch) {
          if (!sessionDirUsed) {
            return json(res, 400, { success: false, error: 'No se encontró sessionDir para este subbot' })
          }
          const patch = {
            name: (typeof body.customName === 'string' ? body.customName : body.custom_name),
            prefix: (typeof body.customPrefix === 'string' ? body.customPrefix : body.custom_prefix),
            banner: (typeof body.customBanner === 'string' ? body.customBanner : body.custom_banner),
            video: (typeof body.customVideo === 'string' ? body.customVideo : body.custom_video),
          }
          const written = writeSubbotConfig(sessionDirUsed, patch)
          if (!written.ok) {
            return json(res, 500, { success: false, error: `No se pudo guardar config.json: ${written.error}` })
          }

          updates.customName = safeString(patch.name).trim()
          updates.customPrefix = safeString(patch.prefix).trim()
          updates.customBanner = safeString(patch.banner).trim()
          updates.customVideo = safeString(patch.video).trim()

          // Aplicar en caliente si esta online
          if (online && sock) {
            try {
              sock.subbotRuntimeConfig = sock.subbotRuntimeConfig || {}
              if (typeof patch.name === 'string') sock.subbotRuntimeConfig.name = safeString(patch.name).trim()
              if (typeof patch.banner === 'string') sock.subbotRuntimeConfig.banner = safeString(patch.banner).trim()
              if (typeof patch.video === 'string') sock.subbotRuntimeConfig.video = safeString(patch.video).trim()
              if (typeof patch.prefix === 'string') {
                sock.subbotRuntimeConfig.prefix = safeString(patch.prefix).trim()
                const pref = normalizePrefixRuntime(sock.subbotRuntimeConfig.prefix)
        ...
        const payload = normalizeSubbotForPanel(record, { isOnline: online })
        emitSubbotUpdated(payload)

        // Enviar notificación de actualización
        try {
          if (typeof global.sendTemplateNotification === 'function') {
            global.sendTemplateNotification('subbot_updated', { subbotCode: record.codigo || record.code });
          }
        } catch (err) {
          console.warn('Error enviando notificación subbot_updated:', err.message);
        }

        return json(res, 200, { success: true, subbot: payload, updates })
      }
...
Panel: ${link}`,
      ...
      if (proveedorLibraryUploadMatch && method === 'POST') {
        ...
        const season = typeof classified?.season !== 'undefined' ? classified.season : null
      ...
      if (pathname === '/api/proveedores' && method === 'POST') {
        ...
        const defaultTiposArchivo = ['PDF', 'EPUB', 'JPG', 'JPEG', 'PNG', 'WEBP', 'MP4', 'ZIP', 'RAR', '7Z', 'CBZ', 'CBR']
        const record = {
          id,
          jid,
          nombre: safeString(body?.nombre || jid),
          ...
          tipos_archivo: Array.isArray(body?.tipos_archivo) ? body.tipos_archivo : (existing?.tipos_archivo || defaultTiposArchivo),
      ...
      if (pathname === '/api/usuarios' && method === 'POST') {
        ...
        if (notifyCredentials) {
          ...
          const hasEmail = Boolean(safeString(targetUser?.email || targetUser?.correo || '').trim())

          if (hasEmail) {
            try {
              const { getEmailServiceStatus } = await import('./email-service.js')
              if (!getEmailServiceStatus()?.configured) {
                warnings.push('SMTP no está configurado; no se enviaron credenciales por email automáticamente.')
              }
            } catch { }
          }

          if (!hasEmail) {
            warnings.push('Usuario creado sin email; no se pudieron enviar credenciales automáticamente.')
          }
        ...
        const responseUser = sanitizeJwtUsuario(auth.usuarios[createdId])
      ...
      if (usuarioIdMatch && (method === 'PATCH' || method === 'PUT')) {
        ...
        if ('email' in (updateData || {}) || 'correo' in (updateData || {})) {
          auth.usuarios[userKey].email = finalEmail || null
        }
        ...
        try {
          if (typeof global.sendTemplateNotification === 'function') {
            global.sendTemplateNotification('user_updated', {
              username: safeString(auth.usuarios[userKey]?.username)
            });
          }
        } catch { }
        ...
        if (beforeRol && afterRol && beforeRol !== afterRol) {
          // Notificación de rol actualizado
          try {
            if (typeof global.sendTemplateNotification === 'function') {
              global.sendTemplateNotification('role_updated', {
                username: safeString(auth.usuarios[userKey]?.username),
                oldRole: beforeRol,
                newRole: afterRol
              });
      ...
      if (usuarioIdMatch && method === 'DELETE') {
        ...
        if (user && auth.usuarios[userKey]) {
          ...
          if (typeof global.sendTemplateNotification === 'function') {
            global.sendTemplateNotification('user_deleted', {
      ...
      if (usuarioPasswordMatch && method === 'POST') {
        ...
        try {
          emitNotification({
            ...
            message: `La contraseña del usuario "${user.username}" ha sido actualizada correctamente.`,
            categoria: 'usuarios'
          })
        } catch { }
      ...
      if (usuarioBanMatch && method === 'POST') {
        ...
        emitNotification({
          ...
          title: banned ? 'Usuario Baneado del Bot' : 'Usuario Desbaneado',
          message: `El usuario ${jid.split('@')[0]} ha sido ${banned ? 'baneado del bot' : 'desbaneado'}${reason ? `. Razón: ${reason}` : '.'}.`,
          categoria: 'usuarios'
        })
      ...
      if (usuarioPremiumMatch && method === 'POST') {
        ...
        emitNotification({
          type: 'success',
          title: premium ? 'Membresía Premium Activada' : 'Membresía Premium Desactivada',
          message: `El usuario ${jid.split('@')[0]} ${premium ? `ahora tiene acceso premium por ${days} día${days !== 1 ? 's' : ''}` : 'ya no tiene acceso premium'}.`,
          categoria: 'usuarios'
        })
      ...
      // ===== Logs =====
      // Terminal (stdout/stderr) en vivo
      if (pathname === '/api/terminal' && method === 'GET') {
        const limit = parseInt(url.searchParams.get('limit') || '200', 10)
        try {
          const { getTerminalLines } = await import('./terminal-mirror.js')
          const lines = getTerminalLines(Number.isFinite(limit) ? limit : 200)
      ...
      if (pathname === '/api/terminal/clear' && method === 'POST') {
        try {
          const { clearTerminalLines } = await import('./terminal-mirror.js')
          clearTerminalLines()
          return json(res, 200, { success: true })
        } catch (error) {
          return json(res, 500, { error: error?.message || String(error) })
      ...
      if (pathname === '/api/system/health' && method === 'GET') {
        ...
        try {
          let dbHealthy = false
          let dbService = global.db?.data ? 'healthy' : 'disconnected'
          ...
          if (typeof global.db?.pool?.query === 'function') {
            ...
            // Fallback: hay data en memoria pero no hay pool (modo legacy)
            dbHealthy = true
          }
      ...
      if (pathname === '/api/tasks' && method === 'POST') {
        ...
        try {
          ...
          await sendNotification({
            ...
            titulo: 'Tarea Creada',
            mensaje: `Nueva tarea programada: ${task.name}`,
            category: NOTIFICATION_CATEGORIES.SYSTEM,
            categoria: NOTIFICATION_CATEGORIES.SYSTEM,
      ...
      if (pathname === '/api/notificaciones/mark-all-read' && method === 'POST') {
        ...
        for (const id in panelDb.notifications) {
          ...
          const isRead = notification?.leida === true || notification?.read === true
          if (isRead) continue
      ...
      if (pathname === '/api/notificaciones/categories' && method === 'GET') {
        ...
        const list = Object.values(panelDb?.notifications || {})
        const role = safeString(auth.user?.rol || '').toLowerCase()
        ...
        const categories = [...new Set(visible.map((n) => safeString(n?.category || n?.categoria)).filter(Boolean))]
      ...
      if (pathname === '/api/notificaciones/types' && method === 'GET') {
        ...
        const types = [...new Set(visible.map((n) => safeString(n?.type || n?.tipo)).filter(Boolean))]
      ...
      if (pathname === '/api/notificaciones/stats' && method === 'GET') {
        ...
        const no_leidas = visible.filter((n) => !(n?.read === true || n?.leida === true)).length
      ...
      if (pathname === '/api/notificaciones' && method === 'GET') {
        ...
        const search = safeString(url.searchParams.get('search') || '').toLowerCase()
        const type = safeString(url.searchParams.get('type') || 'all')
        const category = safeString(url.searchParams.get('category') || 'all')
        const read = safeString(url.searchParams.get('read') || 'all')
        const wantsPagination = url.searchParams.has('page') || url.searchParams.has('limit')

        let notifications = Object.values(panelDb?.notifications || {})

        // Filtrar según parámetros
        const filtered = notifications.filter((n) => {
          if (search) {
            const hay = `${n.title || n.titulo || ''} ${n.message || n.mensaje || ''}`.toLowerCase()
            if (!hay.includes(search)) return false
          }
          if (type && type !== 'all' && (n.type || n.tipo) !== type) return false
          if (category && category !== 'all' && (n.category || n.categoria) !== category) return false
          if (read && read !== 'all') {
            const wantRead = read === 'read'
            if (((n.read || n.leida) !== true) === wantRead) return false
          }

          // Solo mostrar notificaciones públicas o del usuario (si no es admin)
          if (auth.user.rol !== 'owner' && auth.user.rol !== 'admin') {
            if (n.user_id && n.user_id !== auth.user.id) return false
          }
        ...
        const toTs = (n) => {
          const raw = safeString(n?.timestamp || n?.fecha_creacion || n?.created_at || n?.updated_at || '')
        ...
        if (wantsPagination) {
          ...
          return json(res, 200, { notificaciones: items, notifications: items, pagination })
        }

        return json(res, 200, { notificaciones: filtered, notifications: filtered })
      }
      ...
      if (pathname === '/api/notificaciones' && method === 'POST') {
        ...
        const record = {
          id,
          title: safeString(body?.title || body?.titulo || 'Notificación'),
          message: safeString(body?.message || body?.mensaje || ''),
          type: safeString(body?.type || body?.tipo || 'info'),
          category: safeString(body?.category || body?.categoria || 'general'),
          read: false,
          ...
          titulo: safeString(body?.titulo || body?.title || 'Notificación'),
          mensaje: safeString(body?.mensaje || body?.message || ''),
          tipo: safeString(body?.tipo || body?.type || 'info'),
          categoria: safeString(body?.categoria || body?.category || 'general'),
          fecha_creacion: now,
          leida: false
        }
        ...
        // Evitar que el blob crezca indefinidamente: mantener solo las últimas 1000 por ID
        try {
          const keys = Object.keys(panelDb.notifications || {})
          if (keys.length > 1000) {
            const ids = keys
              .map((k) => Number(k))
              .filter((n) => Number.isFinite(n))
              .sort((a, b) => b - a)
            for (const dropId of ids.slice(1000)) {
              delete panelDb.notifications[dropId]
            }
          }
        } catch { }
        ...
        return json(res, 201, { success: true, notification: record })
      }
      ...
      if (pathname === '/api/notificaciones' && method === 'DELETE') {
        ...
        const isAdmin = role === 'owner' || role === 'admin'
        if (!isAdmin) {
          return json(res, 403, { error: 'No tienes permisos para eliminar todas las notificaciones' })
        }

        const confirm = safeString(url.searchParams.get('confirm') || '')
        if (confirm !== 'true' && confirm !== '1') {
          return json(res, 400, { error: 'Confirmación requerida (confirm=true)' })
        }

        if (!panelDb) return json(res, 500, { error: 'DB no disponible' })

        const scope = safeString(url.searchParams.get('scope') || 'all').toLowerCase()
        if (scope !== 'all' && scope !== 'read' && scope !== 'unread') {
          return json(res, 400, { error: 'Scope inválido (all|read|unread)' })
        }

        panelDb.notifications ||= {}

        const ids = Object.keys(panelDb.notifications)
        let deletedCount = 0

        for (const id of ids) {
          const n = panelDb.notifications[id]
          const isRead = n?.read === true || n?.leida === true
          if (scope === 'read' && !isRead) continue
          if (scope === 'unread' && isRead) continue
          delete panelDb.notifications[id]
          deletedCount++
        }
      ...
      if (pathname === '/api/broadcast' && method === 'POST') {
        ...
        if (!auth.ok) return json(res, auth.status, { error: auth.error })
        if (auth.user.rol !== 'owner' && auth.user.rol !== 'admin') {
          return json(res, 403, { error: 'No tienes permisos para enviar mensajes globales' })
        }
        ...
        try {
          ...
          if (!msg) {
            return json(res, 400, { error: 'El mensaje es requerido' })
          }

          // Guardrail: avoid accidental broadcasts with placeholder strings.
          if (/^(undefined|null)$/i.test(msg)) {
            return json(res, 400, { error: 'El mensaje es invalido' })
          }

          // 3. Verificar Conexión
          const conn = global.conn
          if (!conn) return json(res, 503, { error: 'El bot principal no está conectado' })

          // 4. Sincronizar grupos (Fallback seguro si syncGroupsSafe no existe)
          try {
            if (typeof syncGroupsSafe === 'function') {
              await syncGroupsSafe(panelDb)
            } else if (typeof syncGroups === 'function') {
              await syncGroups(panelDb).catch(console.error)
            }
          } catch (e) {
            console.error('Warning: No se pudieron sincronizar grupos antes del broadcast', e)
          }

          // 5. Preparar destinatarios
          let jids = []
          const stats = { groups: 0, channels: 0, communities: 0, specific: 0, broadcasts: 0 }

          // Obtener todos los grupos de la DB
          const allGroups = Object.values(panelDb?.groups || {})

          // Función mejorada para detectar tipo de chat usando metadata de DB
          const detectChatType = (jid, groupData) => {
            if (!jid) return null
            // Usar metadata de la DB si está disponible
            if (groupData?.tipo) return groupData.tipo
            if (groupData?.isChannel) return 'channel'
            if (groupData?.isCommunity) return 'community'
          ...
          const sendToAll = !targets || (typeof targets === 'object' && Object.keys(targets).length === 0)

          // Safety: do not allow implicit "send to all" without an explicit confirmation.
          if (sendToAll && body?.confirmAll !== true) {
            return json(res, 400, { error: 'Confirmación requerida para difusión global (confirmAll=true)' })
          }

          const specificList =
            targetObj && Array.isArray(targetObj.specific)
              ? targetObj.specific.filter(Boolean)
              : []
          const includeCategories = targetObj && targetObj.includeCategories === true

          // If specific targets are provided, treat them as the explicit list by default.
          // This prevents accidental "I selected 1 group but it sent to all" scenarios.
          if (specificList.length > 0 && !includeCategories) {
            jids = [...new Set(specificList)]
            stats.specific = jids.length
          } else if (sendToAll) {
            // Enviar a TODO lo que haya en base de datos
            for (const group of allGroups) {
              const jid = group.wa_jid || group.jid
              const type = detectChatType(jid, group)
              if (jid && type) {
                jids.push(jid)
                // Clasificación mejorada para estadísticas
                if (type === 'group') stats.groups++
                else if (type === 'channel') stats.channels++
                else if (type === 'community') stats.communities++
                else stats.broadcasts++
              }
            }
          } else {
            // Procesar Grupos (ahora con distinción de comunidades)
            if (targetObj.groups === true) {
              for (const group of allGroups) {
                const jid = group.wa_jid || group.jid
                const type = detectChatType(jid, group)
                if (type === 'group') {
            ...
            // Procesar Comunidades (ahora separadas de grupos)
            if (targetObj.communities === true) {
              for (const group of allGroups) {
                const jid = group.wa_jid || group.jid
                const type = detectChatType(jid, group)
                if (type === 'community') {
            ...
            // Procesar JIDs específicos
            if (specificList.length) {
              for (const jid of specificList) {
          ...
          jids = [...new Set(jids)].filter(Boolean)

          if (jids.length === 0) {
            return json(res, 400, { error: 'No se encontraron destinatarios válidos según los filtros seleccionados' })
          }

          // 6. Ejecución en Segundo Plano (Fire and Forget)
          (async () => {
            let successCount = 0
            let failCount = 0
            const total = jids.length

            console.log(`🚀 Iniciando broadcast a ${total} chats...`)

            for (const [index, jid] of jids.entries()) {
              try {
                // Verificar conexión antes de cada envío crítico
                if (!global.conn) throw new Error('Bot desconectado durante el proceso')

                await global.conn.sendMessage(jid, { text: msg })
                successCount++

                // Delay anti-ban: 1.5s a 2s aleatorio
                const delay = Math.floor(Math.random() * 500) + 1500
                await new Promise(resolve => setTimeout(resolve, delay))

              } catch (err) {
                console.error(`❌ Error broadcast ${index + 1}/${total} (${jid}):`, err.message)
                failCount++
              }
            }

            // Notificar finalización (si tienes sistema de notificaciones)
            if (typeof emitNotification === 'function') {
              emitNotification({
                type: successCount > 0 ? 'success' : 'error',
                title: '📢 Broadcast Finalizado',
                message: `Enviados: ${successCount} | Fallidos: ${failCount}`,
                category: 'system'
              })
            }
            console.log(`🏁 Broadcast terminado. Éxito: ${successCount}, Error: ${failCount}`)
          })()

          // 7. Respuesta inmediata al cliente
          return json(res, 200, {
            success: true,
            message: 'El envío masivo ha comenzado en segundo plano.',
            details: {
              total_targets: jids.length,
              breakdown: stats
            }
          })

        } catch (error) {
          console.error('Error fatal en broadcast:', error)
          return json(res, 500, { error: 'Error interno al procesar la solicitud: ' + error.message })
        }
      }
      ...
      if (pathname === '/api/broadcast/email' && method === 'POST') {
        ...
        if (!auth.ok) return json(res, auth.status, { error: auth.error })
        if (auth.user.rol !== 'owner' && auth.user.rol !== 'admin') {
          return json(res, 403, { error: 'No tienes permisos para enviar broadcasts por email' })
        }

        try {
          const body = await readJson(req).catch(() => ({}))
          const { subject, preheader, title, message } = body

          if (!subject?.trim()) {
            return json(res, 400, { error: 'El asunto es requerido' })
          }

          // Obtener emails de todos los usuarios registrados
          const allUsers = auth.usuarios || {}
          const recipients = Object.values(allUsers)
            .filter(u => u?.email || u?.correo)
            .map(u => u.email || u.correo)
            .filter(Boolean)

          if (recipients.length === 0) {
            return json(res, 400, { error: 'No hay usuarios con email registrados' })
          }

          const { sendBroadcastEmail } = await import('./email-service.js')
          const result = await sendBroadcastEmail({
            subject,
            preheader: preheader || message?.substring(0, 100) || '',
            title: title || subject,
            contentHtml: message || '',
            recipients,
            category: 'broadcast',
          ...
          return json(res, 200, { success: true, result, recipientsCount: recipients.length })
        } catch (error) {
          console.error('Error en broadcast email:', error)
          return json(res, 500, { error: 'Error al enviar email: ' + error.message })
        }
      }
      ...
      if (pathname === '/api/broadcast/push' && method === 'POST') {
        ...
        if (!auth.ok) return json(res, auth.status, { error: auth.error })
        if (auth.user.rol !== 'owner' && auth.user.rol !== 'admin') {
          return json(res, 403, { error: 'No tienes permisos para enviar notificaciones push' })
        }

        try {
          const body = await readJson(req).catch(() => ({}))
          const { title, body: pushBody, data, tag } = body

          if (!title?.trim()) {
            return json(res, 400, { error: 'El título es requerido' })
          }

          const { sendPushBroadcast } = await import('./email-service.js')
          ...
          console.error('Error en push broadcast:', error)
          return json(res, 500, { error: 'Error al enviar push: ' + error.message })
        }
      }

      // Full Broadcast (Email + Push)
      if (pathname === '/api/broadcast/full' && method === 'POST') {
        const auth = await getJwtAuth(req)
        if (!auth.ok) return json(res, auth.status, { error: auth.error })
        if (auth.user.rol !== 'owner' && auth.user.rol !== 'admin') {
          return json(res, 403, { error: 'No tienes permisos para enviar broadcasts' })
        }
        ...
        try {
          ...
          // Obtener emails de todos los usuarios registrados
          const allUsers = auth.usuarios || {}
          const emailRecipients = Object.values(allUsers)
            .filter(u => u?.email || u?.correo)
            .map(u => u.email || u.correo)
            .filter(Boolean)

          const { sendFullBroadcast } = await import('./email-service.js')
          const result = await sendFullBroadcast({
          ...
          return json(res, 200, { success: true, result, recipientsCount: emailRecipients.length })
        } catch (error) {
          console.error('Error en full broadcast:', error)
          return json(res, 500, { error: 'Error al enviar broadcast: ' + error.message })
        }
      }

      // Multimedia upload route (both /api/multimedia and /api/multimedia/upload)
      if ((pathname === '/api/multimedia' || pathname === '/api/multimedia/upload') && method === 'POST') {
        ...
        try {
          ...
          return json(res, 500, { error: 'Error al subir archivo' })
        }
      }
      ...
      const multimediaFileMatch = pathname.match(/^\/api\/multimedia\/(\d+)\/file$/)
      if (multimediaFileMatch && method === 'GET') {
        const id = Number(multimediaFileMatch[1])
        const item = panelDb?.multimedia?.[id]
        if (!item) return json(res, 404, { error: 'Archivo no encontrado' })
        const filePath = item.file_path
        if (!filePath || !fs.existsSync(filePath) || !fs.statSync(filePath).isFile()) {
          return json(res, 404, { error: 'Archivo físico no encontrado' })
        }
        const contentType = item.mimeType || mime.lookup(filePath) || 'application/octet-stream'
        res.statusCode = 200
        res.setHeader('Content-Type', contentType)
      ...
      if (pathname === '/api/multimedia' && method === 'GET') {
        ...
        const type = safeString(url.searchParams.get('type') || '')

        const allMultimedia = Object.values(panelDb?.multimedia || {})

        // Filter results
        let filtered = allMultimedia.filter(item => {
        ...
        const sanitizedResults = paginated.map(item => {
          const { file_path, ...safeItem } = item
          return safeItem
        })
      ...
      if (pathname === '/api/multimedia/stats' && method === 'GET') {
        ...
        const stats = {
          total: allMultimedia.length,
          byType: {
            image: allMultimedia.filter(item => item.type === 'image').length,
            video: allMultimedia.filter(item => item.type === 'video').length,
            audio: allMultimedia.filter(item => item.type === 'audio').length,
            document: allMultimedia.filter(item => item.type === 'document').length
          },
          totalSize: allMultimedia.reduce((sum, item) => sum + (item.size || 0), 0),
          recentUploads: allMultimedia
            .sort((a, b) => new Date(b.uploadedAt || b.created_at || '').getTime() - new Date(a.uploadedAt || a.created_at || '').getTime())
            .slice(0, 5)
            .map(item => ({
              id: item.id,
              name: item.name,
      ...
      if (pathname === '/api/resources/stats' && method === 'GET') {
        ...
        try {
          ...
          let stats = resourceMonitor.getStats()

          // Enhance with real-time data from realTimeData for better CPU/memory
          try {
            const rtd = await import('./real-time-data.js')
            const realTime = rtd.default || rtd
            if (realTime?.getSystemStats) {
              const rtdStats = realTime.getSystemStats()
              // Override with more accurate real-time data
              if (rtdStats?.cpu?.usage != null) {
                stats = {
                  ...stats,
                  cpu: { ...stats.cpu, usage: rtdStats.cpu.usage },
                  memory: { ...stats.memory, ...rtdStats.memory },
                  disk: stats.disk, // Keep resource-monitor disk
                }
              }
            }
          } catch (rtdErr) {
            // Ignore real-time data errors, use resource-monitor only
          }
  ...
  // Sistema de limpieza periódica de notificaciones
  const startNotificationMaintenance = async () => {
    try {
      ...
      setInterval(() => {
        try {
          ...
          performNotificationMaintenance(panelDb)
        } catch (error) {
          console.error('❌ Error en mantenimiento de notificaciones:', error)
        }
      }, 6 * 60 * 60 * 1000) // Cada 6 horas

      // Ejecutar una limpieza inicial después de 1 minuto
      setTimeout(() => {
        try {
          const panelDb = ensurePanelDb()
          if (panelDb) performNotificationMaintenance(panelDb)
        } catch (error) {
          console.error('❌ Error en limpieza inicial de notificaciones:', error)
        }
      }, 60000) // 1 minuto

      console.log('🧹 Sistema de limpieza de notificaciones iniciado (cada 6 horas)')
    } catch (error) {
      console.log('⚠️ No se pudo iniciar el sistema de limpieza de notificaciones:', error.message)
    }
  }

  // Iniciar limpieza de notificaciones
  setTimeout(startNotificationMaintenance, 10000)

  // Mantenimiento de SubBots (evita symlinks rotos que causan colisiones)
  const startSubbotMaintenance = async () => {
    try {
      const run = () => {
        try {
          const panelDb = ensurePanelDb()
          const { removed } = cleanupBrokenSubbotSymlinks()
          const normalized = panelDb ? normalizeSubbotSessionDirs(panelDb, { dryRun: false }) : null
          const deduped = panelDb ? dedupePanelSubbots(panelDb) : { removed: 0 }
          if (removed > 0) console.log(`🧹 SubBots: ${removed} symlink(s) roto(s) eliminados`)
          if ((normalized?.migrated || 0) > 0) console.log(`🧩 SubBots: ${normalized.migrated} carpeta(s) renombrada(s) a pushname`)
          if ((normalized?.updated_records || 0) > 0) console.log(`🗂️ SubBots: ${normalized.updated_records} registro(s) actualizados`)
          if ((deduped?.removed || 0) > 0) console.log(`🧽 SubBots: ${deduped.removed} registro(s) duplicado(s) eliminado(s)`)
          if (panelDb && ((normalized?.migrated || 0) > 0 || (normalized?.updated_records || 0) > 0 || (deduped?.removed || 0) > 0)) {
            try { if (global.db?.write) void global.db.write() } catch { }
          }
        } catch { }
      }
      ...
      // Mantenimiento periódico (normalizacion offline + symlinks)
      setInterval(run, 30 * 60 * 1000)

      console.log('🧹 Mantenimiento de SubBots iniciado (cada 30 minutos)')
    } catch (error) {
      console.log('⚠️ No se pudo iniciar mantenimiento de SubBots:', error.message)
    }
  }

  setTimeout(startSubbotMaintenance, 12000)

  // --- Sistema / Recursos ---
  panelServer.post('/api/system/cleanup', async (req, res) => {
    try {
      const results = await performDeepCleanup()
