...
async function syncGroups(panelDb) {
  ...
  for (const group of groups) {
    ...
    // Detectar si es una comunidad basándose en metadata
    const isCommunity = group.isCommunity || group.linkedParent || false
    const isChannel = jid.includes('@newsletter') || jid.includes('@broadcast')
    ...
    if (!panelDb.groups[jid]) {
      ...
      panelDb.groups[jid] = {
        ...
        isChannel: isChannel,
        tipo: isChannel ? 'channel' : (isCommunity ? 'community' : 'group'),
        created_at: now,
        updated_at: now,
        usuario: null,
      }
      ...
      if (existing.nombre !== subject || existing.descripcion !== desc || prevPart !== participantes || (currentBotJid && prevBotJid !== currentBotJid)) {
        ...
        existing.tipo = isChannel ? 'channel' : (isCommunity ? 'community' : 'group')
        ...
        if (forBroadcast) {
          ...
          allGroups.forEach(group => {
            ...
            if (jid.includes('@newsletter')) {
              ...
              // Categorizar como canales o una categoría especial
              channels.push(group)
            } else if (jid.includes('@g.us')) {
              // Por defecto van a grupos, si tuviéramos metadata de comunidad irían a communities
              groups.push(group)
            }
          })
          
          return json(res, 200, { 
            items: allGroups,  // Devolver todos los items en formato compatible con el frontend
            groups,
            channels,
            communities,
