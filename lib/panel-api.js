...
Panel: ${link}`,
      ...
      if (pathname === '/api/broadcast' && method === 'POST') {
        ...
        try {
          ...
          // Obtener la instancia del bot principal
          const conn = global.conn
          if (!conn) return json(res, 503, { error: 'El bot principal no está conectado' })

          let jids = []
          const stats = { groups: 0, channels: 0, communities: 0, specific: 0 }
          ...
          // Función para detectar tipo de chat
          const detectChatType = (jid) => {
            if (!jid) return null
            // Comunidades: terminan en @newsletter
            if (jid.includes('@newsletter')) return 'community'
            // Canales: terminan en @broadcast
            if (jid.includes('@broadcast')) return 'channel'
            // Grupos: terminan en @g.us
            if (jid.includes('@g.us')) return 'group'
            return null
          }

          // Si no hay targets especificados, enviar a todos los grupos
          if (!targets || (typeof targets === 'object' && Object.keys(targets).length === 0)) {
            // Enviar a todos los grupos por defecto
            ...
            // Procesar targets específicos
            if (targets.groups === true) {
              for (const group of allGroups) {
                const jid = group.wa_jid || group.jid
                if (jid && detectChatType(jid) === 'group') {
                  jids.push(jid)
                  stats.groups++
                }
              }
            ...
            if (targets.channels === true) {
              for (const group of allGroups) {
                const jid = group.wa_jid || group.jid
                if (jid && detectChatType(jid) === 'channel') {
                  jids.push(jid)
                  stats.channels++
                }
              }
            ...
            if (targets.communities === true) {
              for (const group of allGroups) {
                const jid = group.wa_jid || group.jid
                if (jid && detectChatType(jid) === 'community') {
                  jids.push(jid)
                  stats.communities++
                }
              }
            ...
            // Agregar JIDs específicos si se proporcionan
            if (targets.specific && Array.isArray(targets.specific)) {
              for (const jid of targets.specific) {
          ...
          // Eliminar duplicados y filtrar JIDs válidos
          jids = [...new Set(jids)].filter(Boolean)

          if (jids.length === 0) {
            return json(res, 400, { error: 'No se seleccionaron destinatarios válidos' })
          }

          // Enviar mensajes (con un pequeño delay para evitar spam/bloqueo)
          let successCount = 0
          let failCount = 0
          ...
          (async () => {
            for (const jid of jids) {
              try {
                await conn.sendMessage(jid, { text: safeString(message) })
                successCount++
                // Delay de 1.5 segundos entre mensajes para evitar bloqueo
                await new Promise(resolve => setTimeout(resolve, 1500))
              } catch (err) {
                console.error(`Error enviando mensaje global a ${jid}:`, err.message)
                failCount++
              }
            }
            ...
            emitNotification({
              type: 'success',
              title: '📢 Mensaje Global Finalizado',
              message: `Se enviaron ${successCount} mensajes exitosamente. Fallidos: ${failCount}. Grupos: ${stats.groups}, Canales: ${stats.channels}, Comunidades: ${stats.communities}`,
              category: 'system'
            })
          })()
          ...
          return json(res, 200, {
            success: true,
            message: 'Envío masivo iniciado en segundo plano',
            estimatedTargets: jids.length,
            stats: stats
          })
        } catch (error) {
          console.error('Error en broadcast:', error)
          return json(res, 500, { error: 'Error interno al procesar el envío masivo' })
        }
      }
