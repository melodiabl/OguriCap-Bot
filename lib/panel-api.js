...
function ensurePanelDb() {
  ...
  panel.systemConfig ||= {
    ...
    autoAddAdminIPOnLogin: false, // Auto-agregar IP al loguear (owner/admin)
  ...
  panelServer = http.createServer(async (req, res) => {
    try {
      ...
      if (pathname === '/api/system/config' && method === 'GET') {
        ...
        if (!isOwner) delete payload.adminIPs
      ...
      if (pathname === '/api/system/add-admin-ip' && method === 'POST') {
        ...
        const exists = adminIPs.some((ip) => normalizeClientIP(ip) && normalizeClientIP(ip) === (norm || clientIP))
        if (!exists) {
          panelDb.systemConfig = panelDb.systemConfig || {}
          panelDb.systemConfig.adminIPs = [...adminIPs, norm || clientIP]
          console.log(`✅ IP agregada como administrador: ${norm || clientIP}`)
          try {
            if (global.db?.write) await global.db.write()
          } catch {}
        }
        ...
        const payload = {
          success: true,
          addedIP: norm || clientIP,
          adminIPsCount: adminIPsFinal.length,
          ...(isOwner ? { adminIPs: adminIPsFinal } : {}),
        }
        return json(res, 200, payload)
      }
