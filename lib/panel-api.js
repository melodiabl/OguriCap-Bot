...
function normalizeSubbotForPanel(record, { isOnline }) {
  ...
  const hasPendingAuth = Boolean((qrDataRaw || pairingCodeRaw) && !sessionInfo.hasCreds)

  const connectionState = isOnline
    ? 'connected'
    : (hasPendingAuth || (sessionInfo.hasSessionDir && !sessionInfo.hasCreds))
      ? 'needs_auth'
...
async function getSubbotsList() {
  ...
  const result = records
    .map((rec) => {
      ...
      if (sessionInfo.hasCreds && (rec.qr_data || rec.qrCode || rec.qr_code || rec.pairingCode || rec.pairing_code)) {
        rec.qr_data = null
        rec.qrCode = null
        rec.qr_code = null
