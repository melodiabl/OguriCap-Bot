...
export async function startPanelApi({ port, host } = {}) {
  ...
  panelServer = http.createServer(async (req, res) => {
    try {
      ...
      // ============================================
      // ENDPOINTS ON-DEMAND - MODO TERMINAL
      // ============================================
      
      // POST /bot/auth/qr - Iniciar autenticación QR bajo demanda
      if ((pathname === '/api/bot/auth/qr' || pathname === '/bot/auth/qr') && method === 'POST') {
        if (typeof global.loadDatabase === 'function') await global.loadDatabase()
        ...
        const currentState = global.botAuthState || 'IDLE'
        const body = await readJson(req).catch(() => ({}))
        ...
        if (currentState === 'AUTHENTICATING' && !force) {
          const currentMethod = global.botAuthMethod || null
          // Si ya es QR, devolver estado (idempotente)
          if (currentMethod === 'qr') {
            return json(res, 200, {
              success: true,
              state: 'AUTHENTICATING',
              method: 'qr',
              message: 'Autenticación QR ya iniciada',
              requestId: global.authRequestId || null,
            })
          }

          // Si esta autenticando con pairing y el panel pide QR, reemplazar automaticamente
          try {
            if (typeof global.closeAuthConnection === 'function') await global.closeAuthConnection()
          } catch {}
        }

        // Si se forzó, intentar cerrar auth previa
        if (currentState === 'AUTHENTICATING' && force) {
          try {
            if (typeof global.closeAuthConnection === 'function') await global.closeAuthConnection()
          } catch {}
        ...
        try {
          // Llamar función on-demand de index.js
          if (typeof global.startQrAuth === 'function') {
            const result = await global.startQrAuth()
            return json(res, 200, {
              success: result.success,
              state: global.botAuthState,
              message: result.success ? 'Autenticación QR iniciada' : result.error,
              requestId: global.authRequestId
            })
          } else {
            return json(res, 500, { error: 'Función de autenticación no disponible' })
          }
        } catch (e) {
          return json(res, 500, { error: e?.message || 'Error iniciando autenticación QR' })
        }
      }

      // POST /bot/auth/pair - Iniciar autenticación por Pairing Code bajo demanda
      if ((pathname === '/api/bot/auth/pair' || pathname === '/bot/auth/pair') && method === 'POST') {
        if (typeof global.loadDatabase === 'function') await global.loadDatabase()
        const panelDbLocal = panelDb || ensurePanelDb()
        if (!panelDbLocal) return json(res, 500, { error: 'DB no disponible' })
        if (!isOwnerOrAdmin(req, url, panelDbLocal)) return json(res, 403, { error: 'Permisos insuficientes' })
        ...
        // Si hay un proceso en curso
        if (currentState === 'AUTHENTICATING' && !force) {
          // Si ya es pairing con el mismo numero, devolver estado (idempotente)
          const currentMethod = global.botAuthMethod || null
          const currentPhone = global.botAuthPhone || null
          if (currentMethod === 'pairing' && currentPhone && String(currentPhone) === String(phoneNumber)) {
            return json(res, 200, {
              success: true,
              state: 'AUTHENTICATING',
              method: 'pairing',
              message: 'Autenticación por pairing ya iniciada',
              requestId: global.authRequestId || null,
              pairingCode: global.panelPairingCode || null,
              displayCode: global.panelPairingCode || null,
              phoneNumber,
            })
          }

          // Si esta autenticando con QR y el panel pide pairing, reemplazar automaticamente
          try {
            if (typeof global.closeAuthConnection === 'function') await global.closeAuthConnection()
          } catch {}
        }
        ...
        try {
          // Llamar función on-demand de index.js
          if (typeof global.startPairingAuth === 'function') {
            const result = await global.startPairingAuth(phoneNumber)
            return json(res, result.success ? 200 : 500, {
              success: result.success,
              state: global.botAuthState,
              message: result.success ? 'Autenticación por pairing iniciada' : result.error,
              requestId: global.authRequestId,
              pairingCode: result.pairingCode || null,
              displayCode: result.displayCode || null,
          ...
          return json(res, 500, { error: e?.message || 'Error iniciando autenticación por pairing' })
        }
      }

      // GET /bot/auth/status - Obtener estado de autenticación
      if ((pathname === '/api/bot/auth/status' || pathname === '/bot/auth/status') && method === 'GET') {
        const currentState = global.botAuthState || 'IDLE'
        const connectionStatus = global.stopped || (currentState === 'CONNECTED' ? 'open' : 'close')
        
        return json(res, 200, {
          state: currentState,
          status: connectionStatus,
          connected: currentState === 'CONNECTED',
          authenticating: currentState === 'AUTHENTICATING',
          idle: currentState === 'IDLE',
          method: global.botAuthMethod || null,
          phone: global.botAuthPhone || null,
          requestId: global.authRequestId || null,
          qrAvailable: currentState === 'AUTHENTICATING' && global.botAuthMethod === 'qr' && global.panelApiMainQr ? true : false,
          pairingCode: global.panelPairingCode || null,
          phoneNumber: global.conn?.user?.id || null
        })
      }

      // POST /bot/auth/disconnect - Desconectar y volver a IDLE
      if ((pathname === '/api/bot/auth/disconnect' || pathname === '/bot/auth/disconnect') && method === 'POST') {
        if (typeof global.loadDatabase === 'function') await global.loadDatabase()
        const panelDbLocal = panelDb || ensurePanelDb()
        if (!panelDbLocal) return json(res, 500, { error: 'DB no disponible' })
        if (!isOwnerOrAdmin(req, url, panelDbLocal)) return json(res, 403, { error: 'Permisos insuficientes' })
        ...
        try {
          if (typeof global.closeAuthConnection === 'function') {
            ...
            return json(res, 500, { error: 'Función de desconexión no disponible' })
          }
        } catch (e) {
          return json(res, 500, { error: e?.message || 'Error desconectando' })
        }
      }
