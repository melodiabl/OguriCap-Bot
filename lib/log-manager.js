...
class LogManager extends EventEmitter {
  ...
  getStats() {
    return {
      ...this.stats,
      uptime: Math.floor(process.uptime()), // segundos reales del proceso Node.js
      bufferSize: this.buffer.length,
      activeStreams: this.currentStreams.size,
      diskUsage: {
        totalSize: 0,
        fileCount: this.stats.filesCreated,
        formattedSize: '0 B'
      }
    };
  }
  ...
  async searchLogs(query, options = {}) {
    // Implementación básica (para evitar que explote si se llama)
    return [];
  }
}
