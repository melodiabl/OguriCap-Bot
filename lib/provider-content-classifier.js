...
async function callAiText(message, model) {
  ...
  if (openaiKey) {
    ...
    const openaiModel = safeString(process.env.PANEL_OPENAI_MODEL_CLASSIFIER || process.env.PANEL_OPENAI_MODEL || 'gpt-4o-mini').trim() || 'gpt-4o-mini'
    const systemPrompt = 'Responde SOLO un JSON válido. Sin markdown. Sin texto extra.'
    ...
    return { content, provider: 'openai', model: openaiModel }
  }

  const allowLegacy = !['0', 'false', 'no', 'off'].includes(safeString(process.env.PANEL_LIBRARY_AI_LEGACY_APIS || process.env.PANEL_AI_LEGACY_APIS || '0').toLowerCase().trim())
  if (!allowLegacy) throw new Error('AI no configurada (falta OPENAI_API_KEY)')
