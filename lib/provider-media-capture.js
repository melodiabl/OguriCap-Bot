...
async function captureOneMessage(conn, msg) {
  ...
  const timeoutMs = Number(process.env.PANEL_PROVIDER_CAPTURE_TIMEOUT_MS || 600000)
  const canStream = typeof conn?.downloadMToFile === 'function'
  const canBuffer = typeof conn?.downloadM === 'function'
  if (!canStream && !canBuffer) return
  ...
  try {
    if (canStream) {
      ...
      const out = await conn.downloadMToFile(content, kind, tmp, { maxBytes, timeoutMs })
      await fs.promises.rename(out?.filename || tmp, filePath)
      ...
      const buffer = await conn.downloadM(content, kind)
