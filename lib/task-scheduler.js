  ...
  async executeTask(taskId, manual = false) {
    ...
    try {
      ...
      if (task.priority >= TASK_PRIORITIES.HIGH) {
        await notificationSystem.sendFromTemplate('task_started', { 
          taskName: task.name 
        });
      }
      ...
      if (task.priority >= TASK_PRIORITIES.HIGH) {
        await notificationSystem.sendFromTemplate('task_completed', { 
          taskName: task.name,
      ...
      if (task.priority >= TASK_PRIORITIES.HIGH) {
        await notificationSystem.sendFromTemplate('task_failed', { 
          taskName: task.name, 
          error: error.message 
        });
      }
      ...
      try {
        ...
        if (adminEmail) {
          ...
          const html = renderPanelEmail({
            ...
            contentHtml,
            ctaUrl: process.env.PANEL_URL || 'https://melodiaauris.qzz.io',
  ...
  async performHealthCheck(config) {
    ...
    if (config.checkMemory) {
      ...
      if (memPercent >= (config.alertThreshold || 90)) {
        await notificationSystem.sendFromTemplate('high_memory_usage', { 
          usage: Math.round(memPercent),
    ...
    if (config.checkCpu !== false) {
      ...
      if (loadPercent >= (config.cpuThreshold || 80)) {
        await notificationSystem.sendFromTemplate('high_cpu_usage', { 
          load: Math.round(loadPercent)
        });
      }
    }
      ...
      try {
        ...
        if (mainFs) {
          ...
          if (usedPercent >= (config.diskThreshold || 85)) {
            await notificationSystem.sendFromTemplate('low_disk_space', {
