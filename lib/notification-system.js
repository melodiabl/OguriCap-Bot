...
class NotificationSystem {
  ...
  generateHash(notification) {
    const content = `${notification.title}|${notification.message}|${notification.category}|${JSON.stringify(notification.to || '')}`;
    let hash = 0;
  ...
  initializeTemplates() {
    ...
    this.templates.set('user_login', {
      ...
      message: 'Un usuario ha iniciado sesión en el panel: {username}',
    ...
    this.templates.set('subbot_connected', {
      ...
      message: 'El subbot {subbotCode} se ha conectado exitosamente',
      type: NOTIFICATION_TYPES.SUCCESS,
      category: NOTIFICATION_CATEGORIES.BOT,
      priority: NOTIFICATION_PRIORITIES.NORMAL,
      channels: [NOTIFICATION_CHANNELS.SOCKET, NOTIFICATION_CHANNELS.BROWSER, NOTIFICATION_CHANNELS.EMAIL]
    ...
    try {
      ...
      if (this.recentHashes.has(hash)) {
        ...
        if (now - lastSent < 60 * 1000) { // No repetir exactamente lo mismo en 1 minuto
          console.log(`🚫 Notificación duplicada omitida (hash: ${hash})`);
          return false;
        }
      }
 ...
 */
async function sendTemplateNotification(templateName, data = {}) {
  const template = notificationSystem.templates.get(templateName);
  if (!template) {
  ...
  const interpolatedTitle = notificationSystem.interpolateMessage(template.title || template.message, data);
  const interpolatedMessage = notificationSystem.interpolateMessage(template.message, data);
  ...
  return await notificationSystem.send(notification);
}

export { sendTemplateNotification };
