// Sistema de Notificaciones (panel + canales: socket/email/webhook/whatsapp)
...
class NotificationSystem {
  ...
  // En este proyecto, "browser" significa mostrarlo en el panel web (via Socket.IO)
  ...
  generateEmailTemplate(notification) {
    ...
    const ts = notification.createdAt || notification.timestamp || notification.fecha_creacion || notification.created_at;
    ...
    const panelUrl =
      ...
      'http://localhost:3000';
    const title = String(notification.title || notification.titulo || '').trim();
    const category = String(notification.category || notification.categoria || '').trim().toUpperCase();
    const type = String(notification.type || notification.tipo || '').trim().toUpperCase();
    const message = String(notification.message || notification.mensaje || '').trim().replace(/\n/g, '<br />');
...
_${new Date().toLocaleString()}_`;
  ...
  async saveNotification(notification) {
    try {
      ...
      // Normalizar claves principales (ingles/es)
      savedNotification.title = String(savedNotification.title || savedNotification.titulo || 'Notificación');
      savedNotification.message = String(savedNotification.message || savedNotification.mensaje || '');
      savedNotification.type = String(savedNotification.type || savedNotification.tipo || NOTIFICATION_TYPES.INFO);
      savedNotification.category = String(savedNotification.category || savedNotification.categoria || NOTIFICATION_CATEGORIES.SYSTEM);
      savedNotification.titulo = String(savedNotification.titulo || savedNotification.title);
      savedNotification.mensaje = String(savedNotification.mensaje || savedNotification.message);
      savedNotification.tipo = String(savedNotification.tipo || savedNotification.type);
      savedNotification.categoria = String(savedNotification.categoria || savedNotification.category);

      const read = Boolean(savedNotification.read || savedNotification.leida || false);
      savedNotification.read = read;
      savedNotification.leida = read;

      const nowIso = new Date().toISOString();
      const createdAt = String(savedNotification.fecha_creacion || savedNotification.timestamp || savedNotification.created_at || nowIso);
      savedNotification.fecha_creacion = createdAt;
      savedNotification.timestamp = String(savedNotification.timestamp || createdAt);
      savedNotification.created_at = String(savedNotification.created_at || createdAt);
      savedNotification.updated_at = String(savedNotification.updated_at || savedNotification.created_at);
