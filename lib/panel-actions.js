...
export async function setBotGlobalState(isOn) {
  if (!global.db?.data) return { success: false, error: 'DB no disponible' }

  global.db.data.botGlobalState = {
    isOn: Boolean(isOn),
...
export function setBotGlobalOffMessage(message) {
  ...
  global.db.data.botGlobalOffMessage = String(message || '')
...
export async function updatePedidoEstado(id, estado) {
  if (!global.db?.data?.pedidos) return { success: false, error: 'DB no disponible' }

  const pedido = global.db.data.pedidos[id]
