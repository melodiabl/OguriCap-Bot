...
export default async function (m, conn = { user: {} }) {
  ...
  if (typeof conn.getName === 'function') {
    try {
      _name = await Promise.resolve(conn.getName(m.sender)).catch(() => null)
    } catch { }
  }
  let sender = '+' + (m.sender || '').replace('@s.whatsapp.net', '') + (_name ? ' ~ ' + _name : '')
  let chat = null
  if (m.chat && typeof conn.getName === 'function') {
    try {
      chat = await Promise.resolve(conn.getName(m.chat)).catch(() => null)
    } catch { }
  }
  let img
  try {
  ...
  let chatName = chat ? (m.isGroup ? 'Grupo ~ ' + chat : 'Privado ~ ' + chat) : (m.isGroup ? 'Grupo ~ Desconocido' : 'Privado')
  let me = '+' + (conn.user?.jid || '').replace('@s.whatsapp.net', '')
  const userName = conn.user.name || conn.user.verifiedName || "Desconocido"
  // Evitar loguear mensajes enviados por el bot, pero sin depender de m.sender (puede variar)
...
${chalk.hex('#FE0041').bold('│')}${chalk.blueBright('Remitente:')} ${chalk.redBright(sender || 'Desconocido')}
${chalk.hex('#FE0041').bold('│')}${chalk.cyanBright(`Chat ${m.isGroup ? 'Grupal' : 'Privado'}:`)} ${chalk.greenBright(chat || (m.isGroup ? 'Desconocido' : 'Privado'))}
...
${chalk.hex('#FE0041').bold('╰───────────────────···')}`)
  ...
  if (typeof m.text === 'string' && m.text) {
    ...
    if (testi && typeof conn.getName === 'function') {
  ...
  if (m.messageStubParameters) {
    console.log(m.messageStubParameters.map(jid => {
      ...
      let name = typeof conn.getName === 'function' ? conn.getName(jid) : null
