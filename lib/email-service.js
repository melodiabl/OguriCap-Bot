...
function getSmtpConfig() {
  ...
  const user = (String(cfgSmtp?.user ?? process.env.SMTP_USER ?? '')).trim()
  // Gmail App Password viene con espacios; normalizar para evitar errores.
  const pass = String(cfgSmtp?.pass ?? process.env.SMTP_PASS ?? '').replace(/\s+/g, '').trim()
...
export async function sendTestEmail({ to }) {
  ...
  if (!resolvedTo) return { ok: false, skipped: false, reason: 'Missing recipient' }
