...
const roundTo = (v, decimals = 2) => {
  ...
  const d = Math.max(0, Math.min(6, safeNumber(decimals, 2)))
  const factor = 10 ** d
...
class RealTimeDataManager {
  ...
  broadcastUpdates() {
    try {
      ...
      emitSystemStats({
        cpu: roundTo(this.systemMetrics.cpu.usage, 2),
        memory: roundTo(this.systemMetrics.memory.usage, 2),
  ...
  // Force immediate update and return fresh data
  forceUpdate() {
    this.updateSystemMetrics()
    this.updateBotMetrics()
    this.updateDatabaseStats()
  ...
  updateSystemMetrics() {
    try {
      ...
      // El panel usa este uptime como "sesion actual", así que debe reflejar el proceso y no el host.
      this.systemMetrics.uptime = Math.max(0, Number(process.uptime()) || 0)
  ...
  getSystemStats() {
    ...
    const cpuUsage = roundTo(this.systemMetrics.cpu?.usage, 2)
    const memUsage = roundTo(this.systemMetrics.memory?.usage, 2)
    const diskUsage = roundTo(this.systemMetrics.disk?.usage, 2)
    const loadAverage = Array.isArray(this.systemMetrics.cpu?.loadAverage)
      ? this.systemMetrics.cpu.loadAverage.map((v) => roundTo(v, 2))
      : []
  ...
  getRecentActivity(limit = 10) {
    ...
    const activities = ordered.map((l) => {
      ...
      const titulo = safeString(l?.titulo || '')
      const detalles = safeString(l?.detalles || '')
      const icon =
        tipo === 'comando' ? 'Zap' :
        tipo === 'mensaje' ? 'MessageSquare' :
        tipo === 'bot' || tipo.includes('bot') ? 'Bot' :
      ...
      const color =
        ...
        tipo === 'bot' || tipo.includes('bot') ? 'info' :
      ...
      return {
        ...
        desc: tipo === 'comando'
          ...
          : `${usuarioShort}${grupoShort ? ` en ${grupoShort}` : ''}${detalles ? ` · ${detalles}` : ''}`.trim(),
        time: toAgo(l?.fecha),
  ...
  getDashboardStats() {
    ...
    const buildHourBuckets = (entries) => {
      const out = {}
    ...
    // Preferir dailyMetrics (más exacto), fallback a logs (si dailyMetrics no se está llenando)
    ...
    // Top commands from logs
    const commandCounts = {}
    for (const l of commandsTodayLogs) {
      const cmd = safeString(l?.metadata?.command || l?.comando || '').replace(/^[^a-zA-Z0-9]/, '').toLowerCase()
      if (cmd) commandCounts[cmd] = (commandCounts[cmd] || 0) + 1
    }
    const topCommands = Object.entries(commandCounts)
      .sort(([, a], [, b]) => b - a)
      .slice(0, 10)
      .map(([command, count]) => ({ command, count, name: command, value: count }))

    // Hourly errors from logs
    const errorBuckets = buildHourBuckets(errorTodayLogs)
    const hourlyErrors = Array.from({ length: 24 }, (_, i) => {
      const h = String(i).padStart(2, '0')
      return { name: `${h}:00`, value: safeNumber(errorBuckets[h], 0), timestamp: `${todayKey}T${h}:00:00.000Z` }
    })

    // Hourly commands
    const cmdBuckets = buildHourBuckets(commandsTodayLogs)
    const hourlyData = Array.from({ length: 24 }, (_, i) => {
      const h = String(i).padStart(2, '0')
      return { name: `${h}:00`, value: safeNumber(cmdBuckets[h], 0), timestamp: `${todayKey}T${h}:00:00.000Z` }
    })
