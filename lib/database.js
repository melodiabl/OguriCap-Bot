...
class Database {
  ...
  async saveData() {
    ...
    // Save chats - batch upsert
    const chatEntries = Object.entries(this.data.chats)
    if (chatEntries.length > 0) {
      const cJids = [], cSettings = [], cMessages = [], cMsgSettings = []
      for (const [jid, chatData] of chatEntries) {
